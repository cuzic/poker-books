...
ハイボードリスクは、ブロードウェイカード（T・J・Q・K・A）が複数並ぶことで相手のレンジが強化される効果と、ペアボードが逆にドライ化する効果を評価します。
...
フラッシュ要素: レインボー → 0
ハイボードリスク: K・T の Broadway 2枚 → −1

BoardScore = 10 − 2 − 0 − 1 = 7
```
...
ストレート要素: バラバラ → 0
フラッシュ要素: レインボー → 0
ハイボードリスク: ペアボード → +1（Broadway K が1枚あるが、ペアルールが優先）
...
| A♠8♠3♦ | 0 | −2 | 0 | **8** | ドライ |
| K♠T♦5♣ | −2 | 0 | −1 | **7** | ドライ |
| J♦8♣4♦ | −2 | −2 | 0 | **6** | セミウェット |
| Q♣J♦9♠ | −3 | 0 | −1 | **6** | セミウェット |
| T♥9♠8♣ | −5 | 0 | 0 | **5** | セミウェット |
| 9♥8♥7♦ | −5 | −2 | 0 | **3** | ウェット |
| J♠T♠9♦ | −5 | −2 | −1 | **2** | ウェット |
| A♥K♥Q♥ | −5 | −5 | −1 | **−1** | 超ウェット |
| 7♦7♣K♥ | 0 | 0 | +1 | **10**（cap） | ドライ |
...
|---|---|---|
| ペアボード（Paired boards） | 16.94% | ドライ寄り（≥ 7） |
