...
本書の判定ルールでは、**H3バケツ（HandScore 15以上）**が「バリューチェックレイズ候補」の目安です。
具体的な対応は次のとおりです。
...
Step 3：後手スコアを計算して判断する（第13章の式）
  ...
  ┌ 後手スコア ≥ 8 かつ HandScore 15以上（H3）
  │        → バリューチェックレイズ（相手ベットの2.5〜3x）
  ├ 後手スコア ≥ 8 かつ ドロー主体（H2 ≥ 8: OESD・FD 等で達成）
...
|--------|--------------|-------------|--------------|
| H3 | 15以上（TP良キッカー〜セット） | ≥ 12 | バリューチェックレイズ |
| H2＋強ドロー | 8〜14 + OESD/FD | 後手スコア ≥ 8 | セミブラフチェックレイズ |
| H2 | 8〜14 ドローなし | 後手スコア 5〜7 | チェックコール |
| H1 | 7以下 | 後手スコア 0〜3 | コール気味（後手下限ライン）|
| H1 | 7以下 | 後手スコア < 0 | フォールド |
...
|-----------|-----------|------------|------|
| H3（≥15） | セット・OP・TPTK・TP良キッカー | **リレイズ（またはオールイン）** | K72r 100%、KJ7r 61%、T98r 52% |
| H2（8-14） | TP弱キッカー・セカンドペア・強ドロー | **コール** | K72r 72%、KJ7r 77%、T98r 44% |
| H1（<8）  | 弱いペア・ドロー・空振り | **フォールド** | K72r 80%、KJ7r 83%、T98r 95% |
...
- **後手スコア ≥ 8（第13章の式）** でCR判断。**H3（HandScore 15以上）** → バリューCR、H2+強ドロー → セミブラフCR。
