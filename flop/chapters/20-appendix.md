...
A: 中級以上では必須です。33% ベットに対して後手下限確率は75%、75% ベットに対して後手下限確率は57%。これを下回るディフェンス頻度は相手にブラフを許します。第13章で扱います。
...
- **後手下限 (Minimum Defense Frequency, MDF)**: 相手のベットに対して少なくとも守るべき頻度。ポット ÷（ポット + ベット）。本書では文脈に応じて3つの言い方を使い分ける：
  - **後手下限ライン**：守る/超える対象としての境界（例「後手下限ラインを下回る」「後手下限ラインを守る」）
  - **後手下限確率**：具体的な数値（例「33% ベットに対する後手下限確率は75%」）
  - **後手下限スコア**：後手スコア式の判断境界として登場する閾値
  - 迷ったら無印「後手下限」が安全。
