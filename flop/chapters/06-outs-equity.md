...
| ドロー | アウツ | HandScore 加点 |
|--------|--------|--------------|
| ガットショット | 4 | **+10** |
| 2オーバーカード | 6 | +7 |
| OESD / ダブルガットショット | 8 | **+14** |
| フラッシュドロー（FD） | 9 | +13 |
| FD ＋ ガットショット | 12 | +14 |
| モンスタードロー（FD ＋ OESD） | 15 | **+17** |

第9章で詳述しますが、**ドロー種別ごとに係数が異なる**ため、単一公式（× 1.5 等）ではなく**表で覚える**方式が GTO 実測と整合します。

特に注意:

- **ガットショット（4 アウツ）+10**:GTO 上の bet 頻度 63.4% で、フラッシュドロー（62.2%）と同等の積極性。実は 4 アウツでも強ドロー扱い
- **OESD（8 アウツ）+14**:bet 頻度 90.5% でメイドハンド級。半端なペアより積極的に bet する
...
- 本書のドロー加点は **ドロー種別ごとの表**（FD+OESD +17 / OESD +14 / FD +13 / ガット +10 等）で覚える
