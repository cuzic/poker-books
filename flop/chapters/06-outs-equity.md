...
前章で学んだ簡易レンジスコア チェックリストは「このフロップでレンジ全体が何%ベットするか」を5秒で算出する手順でした。
本章では、自分の個別ハンドがベット対象になるかを判定するための基礎、つまり HandScore のドロー加点（ドロー種別ごとの値）の数学的根拠を解説します。

フロップでのドロー判断は、長年ポーカープレイヤーを悩ませてきたテーマです。
しかし **Rule of 2 and 4** という単純な近似式を使えば、
アウツ数さえ数えれば完成確率をほぼ暗算できます。
本章の内容を押さえると、HandScore のドロー加点（種別ごとの値）の根拠が明確になります。
...
本書のHandScoreではBDFDに固定値 +4を加算します。フロップ完成までターン・リバーの 2 枚連続が必要なので、現在価値はかなり控えめです。

---

## 6-6　本書のドロー加点とアウツの関係

本書の HandScore では、ドロー加点を **「ドロー種別ごとの表」** で覚える方式を採用しています。これは GTO 実測と整合させるための設計です。

### 単一公式（アウツ × N）が機能しない理由

GTO Wizard 公開データの実測を見ると、**ドロー種別によって bet 頻度の係数が大きく異なる**ことがわかります:

| ドロー種別 | アウツ | GTO 上の bet 頻度 | 換算係数 |
|--------|:------:|:------:|:------:|
| ガットショット | 4 | 63.4% | 約 × 2.5 相当 |
| OESD | 8 | 90.5% | 約 × 1.75 相当 |
| FD | 9 | 62.2% | 約 × 1.4 相当 |
| FD+OESD | 15 | 100.0% | キャップ |

このように単一係数では表現できないため、表で直接覚える方式に統一しています。

### 各ドローの加点まとめ

| ドロー | アウツ | HandScore 加点（フロップ・新スケール）|
|--------|--------|--------------|
| ガットショット | 4 | **+16** (Rule of 4: 4×4) |
| 2オーバーカード | 6 | **+24** (6×4) |
| OESD / ダブルガットショット | 8 | **+32** (8×4) |
| フラッシュドロー（FD） | 9 | **+36** (9×4) |
| FD ＋ ガットショット | 12 | **+48** (12×4) |
| モンスタードロー（FD ＋ OESD） | 15 (重複2控除→13) | **+52** (13×4) |
| BDFD（バックドアフラッシュ） | ─ | **+5**（固定値）|
| BDSD（バックドアストレート） | ─ | **+2**（固定値）|

新スケール（HandScore = 0〜100）では **アウツ × Rule of 4** を直接使います。フロップでは Rule of 4（× 4）、ターンでは Rule of 2（× 2）、リバーでは 0（ドロー確定）です。第9章で詳述します。

特に注意:

- **ガットショット（4 アウツ）+16**: GTO 上の bet 頻度 63.4% で、フラッシュドロー（62.2%）と同等の積極性。実は 4 アウツでも強ドロー扱い
- **OESD（8 アウツ）+32**: bet 頻度 90.5% でメイドハンド級。半端なペアより積極的に bet する
- **モンスタードロー（FD+OESD）+52**: 重複アウツ 2 を控除して 13 アウツ × 4 = 52 で計算
...
このEQR低下と、ドロー種別ごとの戦略的意味（OESD はメイドハンド級、FD はミドル、ガットショットは積極ブラフ）が、本書の表ベースのドロー加点に反映されています。
近似式として使う限り、Rule of 2 and 4 は完成確率の見積もりとして実用的な判断ツールです。
...
- 本書のドロー加点は **アウツ × Rule of 4**（フロップ）で覚える（FD +36 / OESD +32 / ガット +16 / FD+OESD +52 等）
...
**Q3**　HandScore で「フラッシュドロー（9アウツ）＋ ガットショット（4アウツ）」を計算するとき、ドロー加点はいくつになりますか？

<details>
<summary>解答</summary>

ドロー加点表で **「FD + ガットショット」= +14**。

13 アウツのコンボドローですが、係数は 8 アウツ OESD（+14）と同等程度。FD+OESD（15 アウツ）の +17 より少し下になります。表で直接覚える方式により、種別ごとの戦略的意味（積極性）が反映されています。
