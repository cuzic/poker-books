...
```
先手スコア ≥ 7 → ベット方向
  SPR ≤ 2 かつ HandScore ≥ 8（H2 以上）→ オールインを検討
  SPR > 2               → 標準サイズ（33〜75%ポット）

後手スコア ≥ 8 → チェックレイズ方向
  SPR ≤ 2 かつ HandScore ≥ 8（H2 以上）→ オールインチェックレイズを検討
  SPR > 2               → 標準チェックレイズ（2.5〜3倍）

IP CR 応答 H3（HandScore ≥ 15）→ リレイズ方向
  SPR ≤ 2               → オールインリレイズを検討
  SPR > 2               → 標準リレイズ
```
...
- 先手スコア = 18 + 3 = 21 ≥ 7 → ベット方向
- SPR ≤ 2、HandScore 18 ≥ 8（H2 以上）→ **オールインベットを検討**
...
フロップのオールインは SPR ≤ 2〜3 の限られた場面に限定します。SPR ≥ 3 でのオールインはターン・リバー（巻4）でのマルチストリートプランの一部として扱います。
