...
## 15-7　先手/後手スコアとオールインの連携

先手スコア・後手スコアはアクションの**方向**を決め、SPR はそのアクションを**オールインにするか否か**を決めます。

### 基本フロー

```
先手スコア ≥ 40 → ベット方向
  SPR ≤ 2 かつ HandScore ≥ 50（H2 以上）→ オールインを検討
  SPR > 2               → 標準サイズ（33〜75%ポット）

後手スコア ≥ 40 → チェックレイズ方向
  SPR ≤ 2 かつ HandScore ≥ 50（H2 以上）→ オールインチェックレイズを検討
  SPR > 2               → 標準チェックレイズ（2.5〜3倍）

IP CR 応答 H3（HandScore ≥ 65）→ リレイズ方向
  SPR ≤ 2               → オールインリレイズを検討
  SPR > 2               → 標準リレイズ
```
...
| SPR 帯 | オールインに必要な HandScore |
|--------|--------------------------|
| ≤ 2    | H2 以上（≥ 50：トップペア以上） |
| 2〜3   | H3 以上（≥ 65：強トップペア以上） |
| ≥ 3    | オールインは基本なし（ターン・リバーへ） |

SPR が低いほどオールインのハードルが下がります。SPR ≤ 2 では H2（トップペア）でのスタックオフが正当化され、SPR 2〜3 では H3（強TP以上）が必要になります。

### 計算例

**K♣7♦2♠（ドライ）、SPR 2、TPTK（KJ）を持つ IP**

- 先手スコア = 70 + 12 = 82 ≥ 40 → ベット方向
- SPR ≤ 2、HandScore 70 ≥ 50（H2 以上）→ **オールインベットを検討**

**K♣7♦2♠（ドライ）、SPR 5、TPTK（KJ）を持つ IP**

- 先手スコア = 82 ≥ 40 → ベット方向
- SPR 5 > 2 → **標準サイズ（75%ポット程度）でベット、ターンへ**

### SPR ≥ 3 のオールインはターン・リバーへ

フロップのオールインは SPR ≤ 2〜3 の限られた場面に限定します。SPR ≥ 3 でのオールインはターン・リバー（巻④）でのマルチストリートプランの一部として扱います。
...
- **スコア × SPR でオールイン判断**：先手/後手スコアが「方向」、SPR が「オールインか否か」を決める。SPR ≤ 2 かつ H2 以上でオールイン検討可、SPR ≥ 3 のオールインはターン・リバーへ
