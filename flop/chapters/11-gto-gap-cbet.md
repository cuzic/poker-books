...
モノトーンフロップ（3枚が同じマーク、例：K♠8♠3♠）は、本書のBoardScoreでは「フラッシュ要素 +5」が加算され、スコアが大きく上がります。
スコアが高ければ「チェック」判定になりやすい設計です。
...
ペアードフロップ（例：K♥K♣3♠、A♠A♦7♣）は、本書のBoardScoreでは「ペアボード −1」の補正でスコアが下がり、ドライに分類されます。
