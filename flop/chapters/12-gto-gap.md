...
マルチウェイ（3人以上）では、同じ役割カテゴリや同じ簡易レンジスコア頻度帯でもベットの採算が大きく変わります。
...
## 12-2　モノトーンの逆説：小サイズ高頻度が正解

著者の GTO 実測（257 ボード）では、モノトーンフロップの BTN CBet 平均は **70.5%** です。
レインボー（約 55%）や 2tone（約 50%）を大幅に上回り、ペアードボード（約 75%）に次ぐ高頻度帯です。
...
したがってバリューを取れる場面は多く、高頻度ベットが正当化されます。

**理由2：チェックレイズのリスクが低い。**
相手がフラッシュドローを持っている場合は小さいベットにも簡単にコールします。
チェックレイズはフラッシュ完成を確定させてしまうため、相手の最適戦略はコールが主体です。
このため小サイズ（33%）で広いレンジのまま打ち続けるのが GTO の正解です。

**本書の対応：**
本書（第4章）ではこのデータを受けて、型5 モノトーンを「🌊 小打ちモード（33% 広いレンジ）」に改訂しました。
GTO との残存乖離は小さく、主に「弱い手でのチェック判断」の細部のみです。
...
**本書との残存乖離：**
ハイペアード（AA/KK/QQ）の簡易レンジスコア頻度はGTOと整合しましたが、本書式では「捨てる手」はドライ板以外チェックバックします。GTOではAA-X/KK-X上の弱いハンドも含めた**レンジベット**が正解であり、この部分の乖離は残ります。また TT-X（中低ペア 72% vs GTO 55〜60%）は本書がやや高めです。
...
本書の式は純粋戦略（決定論）です。
「攻める手」なら必ずベット、「捨てる手」（ドライ板以外）なら必ずチェックと決まります。
...
| モノトーン | 33% 広いレンジ（小打ちモード）| 70%・33% サイズ | **本書改訂後はほぼ整合** |
...
- GTOとの主な乖離ポイントは「マルチウェイ」「ハンドスコア閾値」の2つ
- ハイペアード（AA/KK/QQ）の頻度は簡易レンジスコアでGTO整合済み
- モノトーンは257ボード実測を受けて「小打ちモード（33% 広いレンジ）」に改訂済み、GTO整合は大幅改善
- マルチウェイは第16章で対処
...
**解答：** GTOはこのボードでBTNから約100%の高頻度でCBetします（小サイズ、ポットの約1/3）。BBがエースを持っている確率が低く、BTNのレンジアドバンテージが絶大だからです。本書式では「捨てる手」（例：ポケットペア中程度）はドライ板以外チェックバックになります。この場面では本書式のほうがチェック頻度が高く、稼ぎ損ねが生じます。

**Q2.** モノトーンフロップで「小サイズで高頻度ベット」が正しい理由を説明し、本書がどのようにこれを反映しているか答えてください。

**解答：** フロップ時点でフラッシュが完成している確率は5〜6%程度にすぎません。大半の相手はフラッシュドローを持っているか、フラッシュ無関係の役だけです。また相手がチェックレイズすると自分のフラッシュ完成が確定してしまうため、相手の最適戦略はコールが主体になります。GTO実測（257ボード）では平均70.5%のCBet頻度が正解です。本書（第4章）ではこのデータを受けて型5を「小打ちモード（33% 広いレンジ）」に改訂しており、GTO整合性は大幅に改善されています。
