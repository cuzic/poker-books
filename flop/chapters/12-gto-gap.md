...
本書のCBet判断フロー（「D3チェックリストで頻度帯を確認し、9マスマトリクスでハンド別アクションを決める」方式）は、GTOソルバーの膨大な出力から「初級者が実行できる形」に圧縮した近似です。
近似には必ずズレが生じます。
...
マルチウェイ（3人以上）では、同じHandScoreや同じD3頻度帯でもベットの採算が大きく変わります。
...
モノトーンフロップ（3枚が同じマーク、例：K♠8♠3♠）は、D3チェックリストではStep 1で即座に「20%（低頻度帯）」と判定されます。
...
ペアードフロップ（例：K♥K♣3♠、A♠A♦7♣）は、D3チェックリストではStep 2のペア表を参照します。KK系は「ハイペア+高キッカー → 80%」または「ハイペア+低キッカー → 50%」と判定され、それぞれ高頻度帯・中頻度帯に分類されます。
...
しかし本書の構造ではD3チェックリスト（テクスチャ評価）が頻度判定の中核に置かれています。
プリフロップレンジとの相互作用は明示的にモデル化されておらず、テクスチャを主ドライバーとして扱っています。

**乖離が顕在化する典型例：**

BTN vs BBでフロップがT♦9♣8♠（ウェット）のとき、D3頻度は低頻度帯（30%程度）となりチェックが推奨されやすくなります。
...
- マルチウェイは第16章で対処。ほかの2つは「稼ぎ損ね」だが大きなミスではない
