...
待って、8♦6♠4♣ はBroadwayなし、1ギャップで連続。ストレート要素を計算し直す：。

- 差 = 4〜8で連続してないが、6と4でギャップ2、8と6でギャップ2。
- 連続2枚＋ギャップ1以内にはあたらない。
- 1ギャップは2箇所存在するが、式では単一値。ここでは −2と簡略化。

..。正確にはBoardScoreはもう少し単純な分類が良いです。以下は本書式の近似値。
...
**A**：本書では **BoardScore 7 がドライ、3 がウェット**の境界。4〜6はセミウェット。ただしこれは近似であり、相手のレンジとの相対で変わります（第22章レンジアドバンテージ）。
...
3人以上のマルチウェイではR4の適用を控え、従来の決定論的サイズ選択（ドライ → 33%、ウェット → 33% かチェック）に戻ります。マルチウェイではフォールドエクイティが下がり、混合サイズの効果が薄れるためです。
...
　　スコア18〜21 → 混合ゾーン（75%/33% の境）→ R2へ
　　スコア ≥ 15 → 強バンド（CBet 33% 確定）
　　スコア11〜14 → 混合ゾーン（33%/チェックの境）→ R2へ
　　スコア ≤ 10 → 弱バンド（チェック確定）
...
- Before（R1のみ）：基本スコア18、弱バンド（17以下）に近い境界 → 33%
- R3適用（ハイカード × レイザー +2）：修正後スコア20、混合ゾーン（18〜21）
- After：R2で高い方（K）のスートが ♠♥ なら **75%**、♣♦ なら **33%**

**例2：ウェットボード　7♦6♣4♠、88（オーバーペア）、レイザー IP**

- Before（R1のみ）：基本スコア23、強バンド → CBet 75%
- R3適用（ローカード × レイザー −2）：修正後スコア21、混合ゾーン（18〜21）
- After：R2で高い方（8）のスートが ♠♥ なら **75%**、♣♦ なら **33%**

**例3：モノトーンボード　K♥9♥5♥、KQ（TPGK）、レイザー IP**

- Before（R1のみ）：基本スコア19（TPGK + ツートーン補正）、混合ゾーン
- R3適用（ハイカード × レイザー +2）：修正後スコア21、混合ゾーン
- After：R2でK（高い方）のスートが ♠♥ なら **75%**、♣♦ なら **33%**
