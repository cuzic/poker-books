...
HandScore = TPTK 18 + ガットショット（4 outs）+6 = **24**。簡易レンジスコア頻度 = KQJr → 連続3枚でウェット要素あり、ハイ×ウェット = **55%**（中頻度帯）。
...
HandScore = TPWK **10**（5キッカー）+ BDFD +4 = **14**。簡易レンジスコア頻度 = K72r → ハイ×ドライ = **85%**（高頻度帯）、補正値0。IP +3。
...
HandScore = 0（空振り）+ ナッツフラッシュドロー（9 outs × 1.5）= 0 + **13**。ブロッカー +3（ナッツAハート）= **16**。簡易レンジスコア頻度 = 982（2-tone）→ ロー×ウェット = **30%**（低頻度帯）、補正値10。IP +3。
...
HandScore = 0 + OESD（8 outs × 1.5）= **12**。簡易レンジスコア頻度 = ロー×ドライ = **55%**（中頻度帯、補正値5）。IP +3。
...
HandScore = 0（空振り）+ 2オーバー（6 outs × 1.5 = 9）= **9**。簡易レンジスコア頻度 = ロー（8）×ウェット（連続2枚+ツートーン）= **30%**（低頻度帯、補正値10）。OOP = 0。3bet補正 +2。
