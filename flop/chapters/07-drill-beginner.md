...
> **本章の目標**: 本書で学んだ3式（D3チェックリスト、HandScore、CBet 統合式）を実戦レベルで運用できる状態に定着させます。20問のドリルで**3秒以内に判断できる**練習をします。
...
<figure><img src="images/f23-fig1-drill-overview.jpg" alt="D3チェックリスト・HandScore・CBetScore・総合問題の4セクション35問ドリル全体像" /><figcaption>D3チェックリスト・HandScore・CBetScore・総合問題の4セクション35問ドリル全体像</figcaption></figure>
...
CBet スコア = HandScore − D3頻度補正値 + ポジション係数（IP +3、OOP 0）
  D3頻度補正値: 高頻度帯→0、中頻度帯→5、低頻度帯→10
...
HandScore = TPTK 18 + BDFDなし = **18**。D3頻度 = K72r → ハイ×ドライ = 85%（高頻度帯）。IP +3。対CBetなのでこの式ではなく、防衛の視点で判断します。
...
HandScore = TPTK 18 + ガットショット（4 outs）+6 = **24**。D3頻度 = KQJr → 連続3枚でウェット要素あり、ハイ×ウェット = **55%**（中頻度帯）。
...
HandScore = OP **20**。D3頻度 = 842r → ロー×ドライ = **55%**（中頻度帯）、補正値5。IP +3。
...
HandScore = OP **20**。D3頻度 = T98（2-tone）→ 連続3枚＋ツートーンでウェット要素あり、ロー×ウェット = **30%**（低頻度帯）、補正値10。IP +3。

CBetスコア = 20 − 10 + 3 = **13 → CBet 33%**（境界）

ウェットボードでQQは**危うい**。すでにストレート完成が相手にある可能性（JQ、JK、67）。チェックで相手の動きを見て守る選択も有力。

**使用した公式**：CBet統合式、D3低頻度帯判定。
...
HandScore = TPTK **18**。D3頻度 = K72r → ハイ×ドライ = **85%**（高頻度帯）、補正値0。IP +3。
...
HandScore = TPWK **10**（5キッカー）+ BDFD +4 = **14**。D3頻度 = K72r → ハイ×ドライ = **85%**（高頻度帯）、補正値0。IP +3。
...
HandScore = 0（空振り）+ ナッツフラッシュドロー（9 outs × 1.5）= 0 + **13**。ブロッカー +3（ナッツAハート）= **16**。D3頻度 = 982（2-tone）→ ロー×ウェット = **30%**（低頻度帯）、補正値10。IP +3。
...
HandScore = 0 + OESD（8 outs × 1.5）= **12**。D3頻度 = ロー×ドライ = **55%**（中頻度帯、補正値5）。IP +3。
...
HandScore = 0（空振り）+ BDFD（クラブは2枚じゃないのでなし）= **0**。D3頻度 = ハイ（A）×ドライ = **85%**（高頻度帯、補正値0）。IP +3。
...
HandScore = セット **30**。D3頻度 = ハイ（K）×ドライ = **85%**（高頻度帯、補正値0）。IP +3。
...
HandScore = セット **30**。D3頻度 = ロー（9）×ウェット（連続3枚+レインボー）= **30%**（低頻度帯、補正値10）。IP +3。
...
HandScore = 空振り（K高、Aハイボード）= 0 + BDFD（クラブ1枚のみなのでなし）= **0**。D3頻度 = ハイ（A）×ドライ = **85%**（高頻度帯、補正値0）。
...
HandScore = TPGK **15**。D3頻度 = ハイ（Q）×ドライ = **85%**（高頻度帯、補正値0）。IP +3。3bet補正 +2。
...
HandScore = 0（空振り）+ 2オーバー（6 outs × 1.5 = 9）= **9**。D3頻度 = ロー（8）×ウェット（連続2枚+ツートーン）= **30%**（低頻度帯、補正値10）。OOP = 0。3bet補正 +2。
...
HandScore = TPTK **18** + ブロッカーなし（ハート1枚もなし）= **18**。D3頻度 = モノトーン = **20%**（補正値10）。IP +3。
...
- **11問以下**：第8章（D3チェックリスト）〜第10章（CBet統合判定）に戻って復習
