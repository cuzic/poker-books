...
M 係数は HU データ（TexasSolver）から理論的に導出した値です（M=12 for 3-way、M=22 for 4-way+、新スケール）。実際のマルチウェイ GTO は MonkerSolver 等で別途確認が必要ですが、実戦では「マルチウェイは閾値が上がる」と覚えるだけで十分です。
...
3ベットポット（SPR 3〜6）でのCBet判断:
  H3（HandScore ≥ 65）: CBet 33%推奨
    → ポットを膨らませてターンでコミット方向へ進む
  H2（HandScore 35〜64）: CBet 25〜33%（選択的）
    → 強めのドロー・セカンドペア良キッカーのみ
  H1（HandScore < 35）: チェック推奨
...
AK on K♠7♦2♣（TPTK、HandScore = 70）の場合、33% CBetを推奨します。このハンドはH3（HandScore ≥ 65）を満たしており、SPR4.8からターンでコミット方向へ進めるポジションにあります。ドライボードで相手のコールレンジが弱い場面では、小さなCBetで価値を積み上げることができます。

**パターン2：H2 × 3ベットポット**

JTs on K♠7♦2♣（ガットショット + 2オーバー、HandScore ≈ 10 + 4×4 = 26、ボトム H2 寄り）の場合、33% セミブラフは選択肢に入りますが選択的です。このHandScoreはH2の下限付近であり、ドロー完成時の強さとSPRを踏まえて判断します。フォールドエクイティが期待できるボード・相手レンジの場合のみ検討し、コール期待値が取れない場合はチェックが無難です。

**パターン3：H1 × 3ベットポット**

66 on K♠7♦2♣（アンダーペア低位、HandScore = 35 だが境界）の場合、チェックを推奨します。H1〜H2 境界のハンドでブラフを打つと、相手はコールするだけでSPR≈2〜3の状態になり事実上コミット済みです。SRPであればレンジベットの一部として打てたこのハンドも、3ベットポットでは打つリスクが大きく上回ります。
...
3ベットポットではSRPのレンジベット戦略が使えません。**H3（HS ≥ 65）のみCBet 33%、H2 は選択的、H1（HS < 35）はチェック**という絞り込みが基本方針です。SRPとの最大の違いは「ブラフのコストがコミット水準に達する」点であり、これを常に意識して判断します。
...
後手スコア = HandScore + A − C − M
  A（ボード補正）: ドライ=+12、セミ=+6、ウェット=0
  C（CBetサイズペナルティ）: 33%→12、50%→17、75%→22、100%→25、150%→30
...
**K♠7♦2♣（ドライ、A=12）、IP が33%（C=12）CBet、HU（M=0）**

| BBのハンド | HandScore | 後手スコア = HS+12−12−0 | 判定 |
|----------|----------|---------------------|------|
| AA（オーバーペア高） | 78 | 78+12−12 = **78** | CR推奨（混合） |
| AK（TPTK） | 70 | 70+12−12 = **70** | CR推奨（混合） |
| JJ（アンダーペア中） | 40 | 40+12−12 = **40** | CR境界 / コール |
| A5s（空振り Aハイ+BDFD） | 25+5=30 | 30+12−12 = **30** | コール |

> **GTO検証注記**: TexasSolverによると、AA（後手スコア78）でのCR頻度は46%、AK（後手スコア70）では34%でした。後手スコア≥40 は「CRを推奨する」ですが、GTOはバランスのためコールも混ぜます。実戦ではCRを主軸にしつつ時々コールを混ぜる混合戦略が最善です。

**T♠9♠8♣（ウェット、A=0）、IP が50%（C=17）CBet、HU（M=0）**

| BBのハンド | HandScore | 後手スコア = HS+0−17−0 | 判定 |
|----------|----------|---------------------|------|
| KK（オーバーペア高） | 78 | 78−17 = **61** | CR推奨（混合） |
| QJ（OESD） | 15 + 32 = 47 | 47−17 = **30** | コール |
| 44（アンダーペア低） | 35 | 35−17 = **18** | フォールド境界（3BP では BB レンジに含まれにくい）|
...
- 式はSRP後手と同じ（後手スコア = HandScore + A − C − M、M=0 for HU）
- BBの3ベットレンジは強いため、H1手は少なく、フォールドは稀
- SPRが低いため、コールすれば次のストリートでほぼコミット
- CR判断（後手スコア ≥ 40）は積極的に実行してよい、コールも重い決断と認識する
...
- ベットサイズは25〜33% 基準
- **M 係数で補正**：先手スコア = HandScore+A−M、後手スコア = HandScore+A−C−M（新スケール、M=12 for 3-way、M=22 for 4-way+）
