...
## 16-8 マルチウェイ用の本書式調整（M 係数）

本書の先手・後手判断式をマルチウェイ補正係数 **M** で統一調整します。
...
| 状況 | M | 先手ベット閾値（先手スコア ≥ 7+M） | 後手CR閾値（後手スコア ≥ 8+M） |
|------|---|------------------------|---------------------|
| HU | 0 | 先手スコア ≥ 7 | 後手スコア ≥ 8 |
| 3-way | 3 | 先手スコア ≥ 10 | 後手スコア ≥ 11 |
| 4-way+ | 6 | 先手スコア ≥ 13 | 後手スコア ≥ 14 |

**計算例（3-way、K72r ドライ A=3、33% CBet C=3）**

- TPTK(HS=18): 先手スコア = 18+3−3 = 18 ≥ 10 → ベット ✓
- アンダーペア(HS=6): 先手スコア = 6+3−3 = 6 < 10 → チェック（HU ではベットできたが 3-way では不可）

**4-way ではさらに厳しくなり** H3（HS≥15）+ ドライ板でようやく 先手スコア ≥ 13 を超えます。

また、HandScore側にも次のハンドタイプ補正を意識します。
