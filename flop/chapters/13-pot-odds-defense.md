...
ポットオッズやMDFは「全体の防衛頻度の目安」です。しかし「この手はコールすべきか、チェックレイズすべきか」という個別判断には、ボードとベットサイズを同時に考慮した式が必要です。本節では **後手スコア** を使い、3択を数値で決める方法を学びます。
...
T♥9♦8♣（ウェット, A=1）、相手が 75% ベット（C=7）。自分の手：8♠8♦（セカンドペア弱、キッカー無関係, HandScore=3）

> 後手スコア = 3 + 1 − 3 − 7 = **−6** → フォールド

**例③ セミウェット板 × 50% CBet**

K♠J♦7♣（セミウェット, A=2）、相手が 50% ベット（C=5）。自分の手：J♦T♦（セカンドペア・キッカー T+BDFD, HandScore=9+4=13）

> 後手スコア = 13 + 2 − 3 − 5 = **7** → コール（境界）

### MDFライン例外：33% ベット時の弱いハンド

33% の小さいベットに対し、OOP は全体の約 75% を守る必要があります（MDFライン）。後手スコアがわずかにマイナスの弱いハンド（アンダーペアや弱いガットショット）も、この頻度を埋めるために一部コールに混ぜます。
...
同じ状況でセカンドペア・良キッカー（HandScore=9）の場合：

> 後手スコア = 9+3−3−3 = 6、後手スコア（調整後） = 6−3 = **3** → コール（HU と同じく CR はしない）
...
- **後手スコア = HandScore + A − 3 − C** でコール/CR/フォールドを決定する
  ...
  - 33% ベット時はMDFライン（約75%防衛）のため 後手スコアが −3 程度までコール気味
...
自分の手は J♥8♥（セカンドペア・キッカー 8 = J 以下なので弱キッカー + BDFD, HandScore = 3 + 4 = 7）です。後手スコアを計算してアクションを決めてください。
...
後手スコア = HandScore + A − 3 − C = 7 + 2 − 3 − 7 = **−1** → MDF ライン上、コール気味（−3 までコール、−4 以下フォールド）

セカンドペア・弱キッカー + BDFD は弱めだが、75% ベットの MDF を埋めるためにコール候補に入る。

**3-way の場合：**

後手スコア（調整後） = 後手スコア − M = −1 − 3 = **−4** → フォールド

3-way になるとマルチウェイ補正で 3 点下がり、フォールド境界に達します。マルチウェイではコールも控え目にしてレンジを絞るのが定石です。
