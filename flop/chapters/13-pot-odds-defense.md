...
- ハンドA：セカンドペア・良キッカー（HandScore 42）、IP
- ハンドB：ガットショット単独（HandScore ≈ 24、空振り 8 + 4 アウツ × 4 = 24）、OOP（BB）
...
HandScore 42 のセカンドペア・良キッカーの推定勝率は約 25〜35% 程度です（HS は equity % に対応）。
IP で必要勝率 25% のボーダーラインにあり、相手のレンジ・ボードテクスチャを考慮しながらコールが視野に入ります。

**ハンドB（ガットショット単独、OOP）**：
HandScore 24（アウツ 4 枚、勝率約 16〜20%）。GTO 上は bet 頻度 63% の積極的なドローですが、**OOP のフェイス側ではフロップでコールよりフォールド優先**。インプライドオッズ（完成時の見返り）が大きい場合のみコール。
