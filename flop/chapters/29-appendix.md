...
### 追加検証：BoardScore と GTO CBet 頻度の相関（30 ボード実測）

公開GTO Wizardデータから代表30フロップ（ドライ・セミウェット・ウェット・ペア・モノトーン各種）を抽出し、本書のBoardScoreとGTO CBet頻度（BTN vs BB SRP）を実測照合しました。

2026年4月実測。`scripts/verify_flop_gto.py` で再現できます。

#### Pearson 相関

**r = −0.774（強い負の相関）**

本書のBoardScoreは「ウェット度の線形予測子」として機能しており、GTO CBet頻度は次の回帰式で近似できます。

```text
GTO CBet 頻度（%） ≈ 71 − 5.8 × BoardScore
```
...
BoardScoreの3段階分類（ドライ ≤3 / セミウェット4–6 / ウェット ≥7）とGTO頻度帯の定性的一致は **19/30 = 63.3%** でした。境界域（BoardScore 3–4や6–7）でGTOと15〜20ポイントずれるケースが散見されます。

#### 主な乖離要因

- **ミドルカードドライ**（J75r、J84ss、Q83ss等）：BoardScoreは低いが、IPのレンジアドバンテージが薄いためGTOは中頻度（50〜60%）
- **ペアボード**（K44、772等）：BoardScoreは最低値だが、ナッツアドバンテージが限定的でGTOは全振りCBetにならない
- **空振り＋ドライ**（AQ on K72r等）：HandScore中心の本書式では「チェック寄り」だが、GTOはRange CBetで高頻度ベット

#### 結論（Level 1）

BoardScoreは **定性的モデルとしては強固**（r = −0.774）で、書籍教材として十分な近似精度を持ちます。ただし3段階分類の絶対的な一致は63% 程度で、境界域ではGTOソルバーとの併用が推奨されます。境界ボードは第27章のR1（3バンド化）で混合戦略に寄せることで、実戦での乖離コストを軽減できます。

### Level 2：上級 BoardScore 補正で精度を上げる

`scripts/verify_flop_gto.py --advanced` で2つの補正を追加すると、Pearson相関・3段階一致率がともに改善します。

#### 補正1：レンジアドバンテージ補正

プリフロップアグレッサー（PFA）のレンジはA・Kを多く含むため、トップカードがQ以下のボードではレンジアドバンテージが薄く、GTOのCBet頻度が本書の基本BoardScore予測より低くなります。トップカードに応じてBoardScoreに加点します。

| トップカード | 補正 | 意味 |
|-----------|------|------|
| A, K | +0 | PFA 強い、基本式通り |
| Q | +1 | やや弱い |
| J, T | +2 | ミッドレンジ、wetness 寄り |
| 9 以下 | 既に straight element で反映 | — |

#### 補正2：ハイペアボード補正

AAx, KKx, QQxのようにhigh kickerを含むペアボードは、PFAのポケットペアヒット確率が高く、実はドライ方向に振れます。現在のBoardScoreは「ペアボード −1」を一律に適用していますが、ハイペアにはさらに **−1** を加算します。

#### Level 2 の実測結果

| 指標 | Level 1 | Level 2 | 改善 |
|------|---------|---------|------|
| 3段階一致率 | 63.3% | **70.0%** | +6.7 pt |
| Pearson 相関 | −0.774 | **−0.822** | +0.048 |
| 線形回帰 傾き | −5.8 | −5.5 | より穏やか |
| 線形回帰 切片 | 70.9 | 72.6 | より高い |

Level 2の回帰式は **GTO 頻度 ≈ 73 − 5.5 × BoardScore** で、低BoardScore側（ドライ）の予測がより高くなり、K72rやA72r（実測90%+）との乖離が縮まります。

#### Level 2 の限界

- J75r、Q83ss、AT7ssなど特定のミッドボードはまだ10〜15ポイント乖離
- 完全一致は連続値のGTO頻度を整数式で近似する限界
- preflopの100% 一致に匹敵する精度はフロップ側では構造上困難

### 全体の結論

**Level 1**（基本式のみ）はr = −0.774で実用十分。
**Level 2**（2補正追加）はr = −0.822で上級者向け。ミッドカードドライボードの過大評価を抑える効果が明確。

検証スクリプト：`scripts/verify_flop_gto.py`（デフォルトはLevel 1、`--advanced`でLevel 2）
データソース：GTO Wizard Blog、Upswing Pokerほかの公開チャート（6-max 100BB BTN vs BB SRP）
