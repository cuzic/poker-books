...
標準的なしきい値（CBetスコア13以上でCBet実行）を基準として、各タイプへの調整を確認します。
...
**しきい値の調整**：CBetスコアのしきい値を13→8まで引き下げ、エアを含む幅広いレンジでCBetを実行します。
...
**実例**：K♠7♥2♣（BoardScore 9、ドライ）でAhJhを持つ場合、HandScore＝0（ハイカードのみ）+9（2オーバーカード）＝9。統合スコア＝9+（9−5）+3＝16で通常はCBet33%の対象です。しかし対Nitなら、エアでも積極的にCBet33%を打ちます。コールされたターンはチェックバックで降りるラインが合理的です。
...
**実例**：T♥9♣8♦（BoardScore 3、ウェット）でT♠A♦を持つ場合、HandScore＝10（トップペア・弱キッカー）。通常はウェットボードでチェックが選択肢に入ります。しかし対コーリングステーションなら、TP以上はすべてCBet75%を打つ方針に切り替えます。
...
**しきい値の調整**：ブラフCBetのしきい値を13→18に引き上げ、バリューベットの手でナッツクラスはトラップ（スロープレー）を選択します。
...
**実例**：8♠7♠6♣（BoardScore 0、非常にウェット）でA♣A♥を持つ場合、標準100BBではCBet33%で手軽にポットをコントロールします。しかしディープ150BBでは、相手がセット、OESD+FD、フラッシュドローを持つ場合のインプライドオッズが高く、チェックバックでポットを小さく保つ選択肢も検討します。
...
**しきい値の変更**：ショートスタックではフロップのCBet統合式よりも「コミットできる強さか否か」の二択判断に移行します。HandScore20以上（オーバーペア相当）ならコミット、それ以下なら慎重なポットコントロールを選択します。
...
| 50〜65% | GTO近似（対処しにくい） | ボードテクスチャと他の観察で補完判断 |
...
- **搾取**：ブラフCBetの頻度を増やす。しきい値をCBetスコア8まで引き下げ、エアでも打つ
...
- スタック深度はSPRで管理し、ディープでは小サイズ・ショートではコミット判断に移行する
...
**Q3**：スタック150BBのディープスタック戦で、フロップに6♥7♥8♦（BoardScore 1、非常にウェット）が出ました。あなたはT♠T♣を持っています。標準100BBと比較して、ディープスタックではどのような点に注意してCBetサイズを選ぶべきでしょうか。
