...
> **補足（第27章への接続）**: 本章のチェックレイズ判定はHandScore基準による決定論ですが、チェックレンジ保護（強ハンドの一部をチェックに回す）の観点は扱っていません。第27章のR5（チェックレンジ保護）では、オフスートのプレミアムハンドをスート判定でチェック候補に回すことで「チェック＝弱い」という等式を崩す方法を扱います。
...
- **HandScore 20以上** → バリューチェックレイズ候補（セット80%頻度、TPTK は小CBet時のみ）。
