...
ここまでの章で、本書のしきい値は固定でした。RFI なら UTG=24、HJ=22、CO=20、BTN=18、SB=22。3-bet なら BTN vs UTG で T_3bet=32 / T_call=28——いつでも同じです。しかし実戦では、相手や状況によってこの数字を**動かす**べき場面があります。
...
例：タイト相手が HJ からオープン → 自分の CO 対 HJ の T_3bet（通常 28）を **30 に上げる**。あるいはオープン側で考えるとき、自分のCO の T_open（通常 20）を **22 に上げる**。
...
例：ルース相手が HJ からオープン → 自分の CO 対 HJ の T_3bet（通常 28）を **26 に下げる**。あるいは自分の CO の T_open（通常 20）を **18 に下げる**。
...
### 大きなオープンへの対応（オープンサイズ読み）

カモ候補は標準の 2.5bb ではなく、5bb・6bb・7bb+ でオープンしてくることがあります。このとき**ポットオッズが悪化**するため、コール損益分岐点も変わります。

#### ポットオッズの変化（BB 視点）

| オープンサイズ | コール額 | 最終ポット | 必要勝率 |
|---|---|---|---|
| 2.5bb（標準） | 1.5bb | 5.5bb | 約 27% |
| 4bb | 3bb | 8.5bb | 約 35% |
| 6bb | 5bb | 12.5bb | 約 40% |
| 8bb | 7bb | 16.5bb | 約 42% |

標準から 3bb 上がるだけで、必要勝率が 13 ポイント悪化します。

#### T_call 調整ルール

```text
T_call_adjusted = T_call + (オープンサイズ − 2.5) × 2
```

- 5bb オープン → T_call + 5
- 7bb オープン → T_call + 9

ただし、相手がルース・パッシブのカモなら**インプライドオッズ**も大きくなります。ナッツを入れたときに大きく取れる期待があるため、調整を +1 per 1bb に緩めるのも実用的です。

#### 3ベット戦略の変化

大きなオープンを打つ相手は「高額を入れたから降りたくない」という心理でコールしやすいです。ブラフ 3ベットの成功率は落ちます。

| 3ベットの種類 | 方針 |
|---|---|
| **ブラフ系**（A5s・KTo など） | 控える（相手がフォールドしない） |
| **バリュー系**（TT+・AQs+・AKo） | 積極的に打つ（コールして大きなポット） |

#### エクスプロイト要約

大きなオープン相手には「**コールを絞り、バリュー 3ベットを広げる**」が最大の搾取です。標準 GTO の混合 3ベット（ブラフ込み）から、純バリュー一択に切り替えます。

---

## 14-3B リンプへのアイソレートレイズ

前のプレイヤーが**リンプ**（BB 同額だけコール）で参加した場合、そのまま見送るか、レイズしてヘッズアップに持ち込むかを判断する必要があります。

### ISO レイズとは

**ISO レイズ**（アイソレートレイズ）は、リンパーをフォールドさせるか 1 対 1 に持ち込むためのレイズです。リンパーは「弱いハンドだが降りたくない」という心理で参加しているため、強いハンドでレイズすれば後ろのプレイヤーを除外しつつ弱いレンジと対峙できます。

### ISO レイズのサイズ

```text
ISO サイズ = 3BB + 1BB × リンパー数
```
...
通常オープン（2.5BB）より大きく設定するのは、すでにポットに入ったコール分を取り込み、後ろのプレイヤーへのオッズを遮断するためです。

### T_iso：ISO レイズの最低スコア

通常の T_open を次のように調整します。

| 状況 | T_iso | 理由 |
|------|-------|------|
| **IP ＋ リンパー 1 人** | T_open **− 2** | リンパーレンジが弱く、ポジション優位を生かせる |
| **IP ＋ リンパー 2 人以上** | T_open（変更なし） | マルチウェイになるため通常基準に戻す |
| **OOP** | T_open（変更なし） | ポジション劣位がポットの拡大を相殺する |

**具体例（BTN、リンパー 1 人）**

T_iso = 18 − 2 = **16**

- 65s：7 + 6 + 3(s) + 1(差1) − 1(両 9 未満) = **16** → ISO レイズ可
- 54s：5 + 4 + 3(s) + 1(差1) − 1(両 9 未満) = **12** → フォールド

**具体例（CO、リンパー 1 人）**

T_iso = 20 − 2 = **18**。BTN と同じ水準まで広げられます。

### BB の特例：無料チェックを活用する

全員がリンプかフォールドして BB に回ってきた場合、BB はチェックで無料参加できます。

- Score_BB ≥ T_iso → **ISO レイズ**でポットを取りに行く
- Score_BB ＜ T_iso → **チェック**して無料でフロップを見る（損失を限定）

### オーバーリンプは原則禁止

リンプが入っているポットに BB 同額だけ追加コール（オーバーリンプ）するのは推奨しません。弱いハンドをポットに入れ続けるうえ、後ろのプレイヤーに有利なオッズを与えます。強いハンドなら ISO レイズ、弱いハンドならフォールドが基本です。
...
- 65s の通常スコア = 7 + 6 + 3 (s) + 1 (差1) − 1 (両カード9未満) = **16**（BTN T_open=18 に届かずフォールド判定）
- ディープ時：S+1（通常 +3 → +4）、C+1（通常 +1 → +2）→ 7 + 6 + 4 + 2 − 1 = **18**
- BTN しきい値 18 にちょうど届き、参加寄りに変わります
...
- A5s のショートスタック3ベット
  - 通常：14 + 5 + 3 (s) + 3 (Aブロッカー) = **25**
  - ショート調整（S 無視 / B+1）：14 + 5 + 4（A=+3 → +4）= **23**
- スーテッドを無視、ブロッカーを重視する運用です。3-bet 域に乗せるには通常スコア 25 → 調整後 23 と下がるので、ショートでは A5s より AKo（Score=32 → ショート調整 32+1=33）のような A・K 両ブロッカー入りが優先候補になります
