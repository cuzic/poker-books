...
スコア32はBTNしきい値18の強バンド（18+3=21以上）なので、R2は不要。スートに関わらず100%オープンです。
...
| 対 SB | **32 / 18** | SB は OOP で BTN と近い開き幅 |

**対SBの防御しきい値について**：SBがオープンレイズした場合、BBのコール下限は BTN と同じ T_call=18 を基準にします。3ベット閾値は T_3bet=32 と高めで、これは SB がオープン時に比較的強いレンジに絞っていることを反映します。

### 3ベット・コール・フォールドの5ゾーン化

BBのディフェンスには 3ベット / コール / フォールドの三択があります。R1では T_3bet と T_call それぞれの周辺に混合ゾーンを設けるため、合計5ゾーン構造になります。

対BTNオープンの場合（T_3bet=28 / T_call=18）：

```text
Score_BB ≥ 31（T_3bet+3）   → 3-bet確定
26〜30（T_3bet±2）          → 3-bet混合ゾーン（R2で3-betorコール）
21〜25                      → コール確定
16〜20（T_call±2）          → コール混合ゾーン（R2でコールorフォールド）
Score_BB ≤ 15（T_call−3）   → フォールド確定
```

混合ゾーンが2か所あることに注意してください。上段（T_3bet周辺）では「3-betかコール」、下段（T_call周辺）では「コールかフォールド」をR2で決定します。

### R3との関係

BBディフェンスの T_call はポジションごとに変化しています（vs UTG: 24 / vs BTN: 18）。この差分自体が相手レンジの強さを反映しており、R3 相当の補正が T_call 表に織り込み済みです。

そのため、BBディフェンスでは R3 を追加適用しません（二重補正になるため）。ポジション別 T_3bet / T_call 表をそのまま使います。
...
- #19の77はスコア24でUTGのT_open=24の混合ゾーン（22〜26）に入ります
...
- BBディフェンスは対ポジション別防御しきい値と5ゾーン化で判断する。対SBは T_3bet=32 / T_call=18（BTNと同じコール下限）
