...
### ✓ DS フレームワーク (turn-defense 全 15 枚) — 旧スケール時代の記録

> **NOTE (2026-05-05)**: 以下は旧スケール (0-30) 時代の audit 記録。
> 現行は新スケール (0-100 equity %) で `DS = HS + A − C − M`、
> SRP 閾値 ≥40 CR / 20-39 CALL / <20 FOLD。
...
td_001〜td_015 全件で「H+A−3−C 計算 → 推奨」が一致。15/15 ✓ (旧スケール時)
...
α 式 (1-α) と V:B 比、ICM・ドロー加点・C 係数（C=3/4/6）は全て検証済み。
