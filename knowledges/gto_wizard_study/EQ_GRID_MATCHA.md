...
equity 計算ができない初心者向けに、自分の手 (カテゴリ) × board の 24-cell grid で
...
| 旧式 (カテゴリ + eq) | 4 値 | 71.02% | 0.34 BB | 1.22% | 中 |

## 暗算の例題 (素人用グリッド経由)

### SRP / 自分 KKx2 (overpair) on Ks-7-2 (dry) / 相手 33% bet
- カテゴリ=`トップペア以上` × board=`dry` → eq=**good** (6 点)
- Score = 6 - 0 + 0 = **6** → **call**

### SRP / AA on 9-8-7 (connected) / 相手 100% bet
- カテゴリ=`トップペア以上` × board=`connected` → eq=**good** (6 点)
- Score = 6 - 2 + 0 = **4** → **call**

### SRP / 22 on K-7-2 (dry, 自分 underpair) / 相手 75% bet
- カテゴリ=`ミドルペア` × board=`dry` → eq=**trash** (0 点)
- Score = 0 - 1 + 0 = **-1** → **fold**
...
- カテゴリ=`エア` × board=`monotone` → eq=**trash** (0 点)
- Score = 0 - 2 + 0 = **-2** → **fold**

## 利用フロー

```
1. 自分の手を見る → 6 カテゴリ のどれか判定 (e.g., overpair / TP+)
