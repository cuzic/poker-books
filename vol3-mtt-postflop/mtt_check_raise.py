#!/usr/bin/env python3
"""
mtt_check_raise.py — MTT フロップ チェックレイズ分析

収集データ:
  1. IP CBet コード取得（フロップ先手）
  2. OOP チェックレイズ率（CBetに対するCR率）
  3. IP の CR 対応（フォールド/コール/3ベット率）

使い方:
  TOKEN=eyJ... GWCLIENTID=xxx python3 mtt_check_raise.py
  TOKEN=eyJ... SBR=40 python3 mtt_check_raise.py
  TOKEN=eyJ... TYPE=型1 python3 mtt_check_raise.py
"""
...
def analyze_board(cfg, ip, oop, pf, depth):
    """1ボードの チェックレイズ 3ステップ分析を実行し、結果 dict を返す。"""
    flop = cfg["flop"]
    ...
    bet_keys  = [k for k in ip_codes if k not in ("check", "fold")]
    total_bet = weighted_rate(ip_rows, bet_keys)
    ...
    if not cbet_code:
        print("    CBetオプションなし（チェックのみ）—— CR分析スキップ")
        return result
...
def main():
    ...
    print(f"分析: フロップ チェックレイズ（OOP CR率 + IP CR対応）")
    print(f"gametype: {GT}\n")
    ...
    # ─── 全体サマリー ───
    print(f"\n\n{'='*70}")
    print(f"★ 全体サマリー: OOP CR率 / IP CR対応")
    print(f"  {'型':20s} | {'OOP CR%':>8} | {'IP fold%':>9} | {'IP call%':>9} | {'IP 3bet%':>9}")
    print(f"  {'-'*65}")
    for r in all_results:
        oc  = r["oop_cr"]
        icr = r["ip_cr_response"]
        cr_s    = f"{oc['cr_pct']:.0f}%"       if oc  and oc["cr_pct"]         is not None else "—"
        fold_s  = f"{icr['fold_pct']:.0f}%"    if icr and icr["fold_pct"]      is not None else "—"
        call_s  = f"{icr['call_pct']:.0f}%"    if icr and icr["call_pct"]      is not None else "—"
        three_s = f"{icr['reraised_pct']:.0f}%" if icr and icr["reraised_pct"] is not None else "—"
        print(
            f"  {r['type']:20s} | {cr_s:>8} | {fold_s:>9} | {call_s:>9} | {three_s:>9}"
        )

    # JSON 保存
    out = FINDINGS_DIR / f"mtt_check_raise_SBR{SBR}.json"
