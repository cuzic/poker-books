...
cash 100bb では HP=2 (air) が 40%、HP=3 (weak pair) が 85%、
HP=5 (second pair) が 98%、HP=7+ (top pair 以上) が 100% continue です。
HP=2 内はキッカー補正 (ace_high +5pt、no_made_hand -3pt) で細分化します。
...
**読み方**: HP が 5 以上なら virtually always continue (98-100%) です。
HP=3 (weak pair) も 85% で continue するため、弱いペアでもほぼ守ります。
判断が必要なのは HP=2 (air) だけです。40% は fold 寄りです。

## Kicker offset — HP=2 の細分化

HP=2 (air) のグループには「ノーペア系」が広く含まれます。
...
**適用条件**: kicker offset は HP=2 のハンドにのみ適用します。
HP=3 以上は kicker offset = 0 で基本 freq をそのまま使います。
...
Step 3: base continue freq 参照
  DCBS_BASE[cash_100bb][HP] → base freq

Step 4: kicker offset (HP=2 のみ)
  HP=2 なら: ace_high +5 / king_high 0 / no_made_hand -3 / low_pair -2
  HP≥3 なら: offset = 0
...
DCBS テーブルの HP 別 continue freq を加重平均すると約 76% になります。

| HP | continue freq | 推定頻度 | 寄与 |
|---:|---:|---:|---:|
| 2 (air) | 40% | 35% | 14% |
| 3 (weak) | 85% | 20% | 17% |
| 5 (mid) | 98% | 20% | 20% |
...
全 4 context の詳細な使い分けは Vol3 (Full DCBS) で扱います。

## まとめ

- HP テーブル参照だけで 90% の判断が完了する。
- HP=2 のみ kicker offset (ace_high +5, no_made_hand -3, low_pair -2)。
- HP≥5 は virtually always continue (98-100%)。
- MDF ≈ 75% と DCBS テーブルは整合している (設計根拠)。
- CR は HP=7+ のバリュー CR + HP=5 以下の稀なブラフ CR のみ。
