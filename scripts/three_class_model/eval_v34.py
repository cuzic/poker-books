        ...
        if pos == "IP":
            ...
            if pot == "3BP":
                if tier == "TP+":
                    ...
                    if bt == "paired" and has_gutshot: return "BET" if tr <= 5 else "CHECK"
                    # v34: paired×has_strong → CHECK (GTO=35.1% e.g. 4s4d2c5h top_pair oesd)
                ...
                if tier == "アンダーペア":
                    ...
                    # v33/v34: wet×third_pair×strong → CHECK (GTO=38.2%) — fix ordering vs wet→BET
                    if bt == "wet" and is_third_pair and has_strong: return "CHECK"
                    if bt == "wet": return "BET"
                    if bt == "paired" and has_none: return "BET" if (4 < tr2 < 10) else "CHECK"
            ...
            if pot == "4BP":
                if tier == "TP+":
                    ...
                    return "BET"
                if tier == "2P+":
                    # v34: wet×set×none → CHECK (GTO=39.3%)
...
df["pred"] = df.apply(predict_v34, axis=1)
...
V33_LOSS = 26.3845; V33_AQS = 86.65
print(f"v34: acc={acc:.3f}%  avg_loss={avg_loss:.4f}%  AQS={score:.2f}")
print(f"vs v33: avg_loss {V33_LOSS:.4f}% -> {avg_loss:.4f}% ({avg_loss-V33_LOSS:+.4f}pp)  AQS {V33_AQS} -> {score:.2f} ({score-V33_AQS:+.2f})")
