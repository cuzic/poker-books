...
def predict_v24(row):
    ...
    if street == "flop":
        if pos == "OOP":
            # v24: OOP 4BP flop — TP+/アンダーペア bet thin value, 2P+ slow play
            if pot == "4BP":
                if tier == "2P+": return "CHECK"
                if tier == "TP+":
                    # overpair slow-plays in 4BP OOP (trap vs strong opponent range)
                    if mv == "overpair": return "CHECK"
                    return "BET"
                if tier == "アンダーペア":
                    # low_pair too weak on wet boards to bet OOP 4BP
                    if bt == "wet" and is_low_pair: return "CHECK"
                    return "BET"
                if tier == "エア": return "BET" if is_ace_high else "CHECK"
                return "CHECK"
        ...
        if pot == "4BP":
            ...
            if tier == "TP+":
                if bt == "dry":
                    # v24: overpair×dry×4BP×flop→CHECK (slow play, better equity realized by trapping)
                    if mv == "overpair": return "CHECK"
        ...
        if pos == "IP":
            ...
            if pot == "4BP":
                ...
                if tier == "アンダーペア":
                    ...
                    # v24: second_pair×paired→CHECK (turn IP 4BP; thin value vs strong ranges)
                    if bt == "paired" and has_none and is_second_pair: return "CHECK"
                ...
                if tier == "エア":
                    ...
                    if bt == "paired":
                        ...
                        # v24: ace_high/king_high×paired×none → BET (blocker/bluff value)
                        if has_none and (is_ace_high or is_king_high): return "BET"
                        return "CHECK"
                return "CHECK"
            if pot == "3BP":
                if tier == "TP+":
                ...
                if tier == "アンダーペア":
                    # v24: underpair×paired → CHECK (turn IP 3BP; too thin)
                    if bt == "paired" and has_none and mv == "underpair": return "CHECK"
            ...
            if pot == "3BP":
                ...
                if tier == "アンダーペア":
                    # v24: low_pair×wet → CHECK (too thin to bet OOP 3BP turn)
                    if bt == "wet" and is_low_pair: return "CHECK"
                    if bt == "wet": return "BET"
                    if bt == "paired" and has_none: return "BET" if (4 < tr2 < 10) else "CHECK"
                if tier == "エア":
            ...
            if pot == "4BP":
                if tier == "TP+":
                    if bt == "wet" and has_none: return "CHECK"
                    # v24: overpair×dry→CHECK (slow play turn OOP 4BP)
                    if bt == "dry" and mv == "overpair": return "CHECK"
                    if bt == "paired":
                        if has_gutshot: return "BET"
                        return "BET" if tr > 10 else "CHECK"
        ...
        if pos == "IP":
            ...
            if pot == "SRP":
                ...
                if tier == "エア":
                    if bt == "paired":
                        # v24: ace_high×paired → BET (bluff); king_high → CHECK
                        if is_king_high: return "CHECK"
            ...
            if pot == "SRP":
                if tier == "TP+":
                    ...
                    return "BET"
                if tier == "2P+":
                    # v24: set×dry → CHECK (slow play on river OOP SRP; induce bluffs)
                    if bt == "dry" and mv == "set": return "CHECK"
                    if bt == "dry" and has_none: return "BET" if tr <= 10 or tr >= 13 else "CHECK"
                    # v24: fullhouse×paired → always BET (strong enough vs any tr)
                    if bt == "paired" and mv == "fullhouse": return "BET"
                    if bt == "paired" and has_none: return "BET" if tr <= 12 else "CHECK"
                    return "BET"
                if tier == "アンダーペア":
                    if bt == "paired":
            ...
            if pot == "3BP":
                ...
                # v24: two_pair×dry → CHECK (trap; too strong for thin river bet OOP 3BP)
                if tier == "2P+" and bt == "dry" and mv == "two_pair": return "CHECK"
            ...
            if pot == "4BP":
                ...
                # v24: ace_high×paired → BET (blocker value)
                if tier == "エア" and bt == "paired" and is_ace_high: return "BET"
...
df["pred"] = df.apply(predict_v24, axis=1)
...
print(f"v24: acc={acc:.3f}%  avg_loss={avg_loss:.4f}%  AQS={score:.2f}")
print(f"vs v23: avg_loss {27.6054:.4f}% -> {avg_loss:.4f}% ({avg_loss-27.6054:+.4f}pp)  AQS 82.17 -> {score:.2f} ({score-82.17:+.2f})")
