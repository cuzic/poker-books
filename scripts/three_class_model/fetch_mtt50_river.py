...
# ///
"""MTT 50bb river defense — use correct turn cbet sizes (R10.6, R2.3)."""
...
# Use R2.3 (smaller turn bet) — keeps SPR moderate for river action
RIVER_BOARDS = [
    ("Kd7c2dKs5h", "K72-K-5"),
    ("Kd7c2dKs9c", "K72-K-9"),
    ("Kd7c2d7s4c", "K72-7-4"),
...
def main() -> int:
    ...
    with httpx.Client(headers=HEADERS) as client:
        for board, label in RIVER_BOARDS:
            chosen_turn = None
            d = None
            # MTT 50bb actual turn sizes: R2.3 (25%), R10.6 (overbet)
            ...
            time.sleep(SLEEP)
            if not bet_actions:
                print(f"  {label}: no river bet actions"); continue
