        ...
        if pos == "IP":
            ...
            if pot == "3BP":
                ...
                if tier == "アンダーペア":
                    ...
                    if bt == "wet": return "BET"
                    # v33: wet×third_pair×strong → CHECK (GTO=38.2%)
                    if bt == "wet" and is_third_pair and has_strong: return "CHECK"
                    if bt == "paired" and has_none: return "BET" if (4 < tr2 < 10) else "CHECK"
...
df["pred"] = df.apply(predict_v33, axis=1)
...
V32_LOSS = 26.4100; V32_AQS = 86.56
print(f"v33: acc={acc:.3f}%  avg_loss={avg_loss:.4f}%  AQS={score:.2f}")
print(f"vs v32: avg_loss {V32_LOSS:.4f}% -> {avg_loss:.4f}% ({avg_loss-V32_LOSS:+.4f}pp)  AQS {V32_AQS} -> {score:.2f} ({score-V32_AQS:+.2f})")
