...
def predict_v36(row):
    ...
    if street == "flop":
        if pos == "OOP":
            ...
            if pot == "4BP":
                if tier == "TP+":
                    ...
                    # v36: wet×top_pair×gutshot → CHECK (GTO=45.8%)
                ...
                if tier == "エア":
                    ...
                    if bt == "paired":
                        ...
                        return "CHECK"
                    # v36: wet×ace_high×strong → BET (GTO=53.7%)
        ...
        # v36: 3BP×dry×set×none = 45.1% BET → CHECK (trap IP dry board set)
        ...
        if pot == "3BP":
            ...
            if tier == "エア":
                ...
                # v36: dry×king_high×strong → BET (53.5%)
        ...
        if pos == "IP":
            if tier == "2P+":
                ...
                # v36: 4BP×dry×straight×none → CHECK (GTO=35.6% — slowplay)
            ...
            if pot == "4BP":
                if tier == "TP+":
                    # v36: paired×overpair×none → CHECK (GTO=48.7% — borderline trap)
                ...
                if tier == "エア":
                    ...
                    if bt == "paired":
                        ...
                        # v31: ace_high×paired×gutshot → BET (58.0%)
            ...
            if pot == "3BP":
                ...
                if tier == "エア":
                    if bt == "dry":
                        ...
                        # v36: gutshot×king_high → CHECK (22.3%)
                ...
                return "CHECK"
            if tier == "TP+":
                # v36: overpair×dry×none → BET (GTO=89%, tr2 8-12 was incorrectly CHECK)
            ...
            if pot == "3BP":
                if tier == "TP+":
                    ...
                    if bt == "paired" and has_gutshot: return "BET" if tr <= 5 else "CHECK"
                    # v36: paired×has_strong → CHECK (GTO=35.1% e.g. 4s4d2c5h top_pair oesd)
                ...
                if tier == "アンダーペア":
                    ...
                    # v33/v36: wet×third_pair×strong → CHECK (GTO=38.2%) — fix ordering vs wet→BET
                    if bt == "wet" and is_third_pair and has_strong: return "CHECK"
                    if bt == "wet": return "BET"
                    if bt == "paired" and has_none: return "BET" if (4 < tr2 < 10) else "CHECK"
                ...
                if tier == "エア":
                    if has_strong:
                        ...
                        # v36: dry×king_high×strong → CHECK (45.4%)
            ...
            if pot == "4BP":
                if tier == "TP+":
                    ...
                    return "BET"
                if tier == "2P+":
                    # v36: wet×set×none → CHECK (GTO=39.3%)
        ...
        if pos == "IP":
            ...
            if pot == "4BP":
                if tier == "TP+": return "BET"
                if tier == "アンダーペア":
                    # v36: dry×underpair × none → BET (GTO=52.9%)
            ...
            if pot == "3BP":
                ...
                # v36: TP+ dry overpair → CHECK (GTO=22.4%, overpair traps on river)
                if tier == "TP+" and bt == "dry" and mv == "overpair" and has_none: return "CHECK"
                return "BET"
            if pot == "4BP":
                if tier == "TP+":
                    # v36: overpair×dry×none → CHECK (GTO=15.9%, trap)
...
df["pred"] = df.apply(predict_v36, axis=1)
...
V35_LOSS = 26.3151; V35_AQS = 86.90
print(f"v36: acc={acc:.3f}%  avg_loss={avg_loss:.4f}%  AQS={score:.2f}")
print(f"vs v33: avg_loss {V35_LOSS:.4f}% -> {avg_loss:.4f}% ({avg_loss-V35_LOSS:+.4f}pp)  AQS {V35_AQS} -> {score:.2f} ({score-V35_AQS:+.2f})")
