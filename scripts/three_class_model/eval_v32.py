        ...
        if pos == "IP":
            ...
            if pot == "4BP":
                ...
                if tier == "エア":
                    ...
                    # v32: threshold 5<tr<11 → tr>=5 to cover low paired boards (tr=5, 66.1% BET)
...
df["pred"] = df.apply(predict_v32, axis=1)
...
V31_LOSS = 26.4397; V31_AQS = 86.45
print(f"v32: acc={acc:.3f}%  avg_loss={avg_loss:.4f}%  AQS={score:.2f}")
print(f"vs v31: avg_loss {V31_LOSS:.4f}% -> {avg_loss:.4f}% ({avg_loss-V31_LOSS:+.4f}pp)  AQS {V31_AQS} -> {score:.2f} ({score-V31_AQS:+.2f})")
