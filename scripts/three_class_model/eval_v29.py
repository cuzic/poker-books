    ...
    if street == "flop":
        if pos == "OOP":
            ...
            if pot == "4BP":
                ...
                if tier == "アンダーペア":
                    ...
                    # v29: wet×third_pair×strong → CHECK (16.5%)
                    if bt == "wet" and is_third_pair and has_strong: return "CHECK"
                    return "BET"                      # second/third pair 60-77%
                if tier == "エア":
                    if bt == "dry":
                        # v29: dry×no_made_hand×draw → BET (gutshot 61%, strong 83.2%)
                        if is_no_made and has_any: return "BET"
                        # v29: dry×king_high×strong → BET (71.3%)
                        if is_king_high and has_strong: return "BET"
                        # v29: dry×ace_high×gutshot → CHECK (35.5%), dry×ace_high×none → BET (57.6%)
        ...
        if pot == "3BP":
            ...
            if tier == "アンダーペア":
                ...
                # v29: wet×second_pair×strong → BET (77.6%)
        ...
        if pos == "IP":
            ...
            if pot == "4BP":
                ...
                if tier == "エア":
                    ...
                    if bt == "paired":
                        ...
                        # v29: king_high×paired×gutshot → BET (86.2%)
                        if is_king_high and has_gutshot: return "BET"
                        # v29: ace_high×paired×gutshot → BET (58.0%)
            ...
            if pot == "3BP":
                ...
                if tier == "アンダーペア":
                    if bt == "paired" and mv == "underpair": return "CHECK"  # v29: underpair 28.2%
            ...
            if pot == "SRP":
                ...
                if tier == "アンダーペア":
                    # v29: paired×second_pair×strong → BET (98.2%)
            ...
            if pot == "3BP":
                ...
                if tier == "アンダーペア":
                    if bt == "wet" and is_low_pair: return "CHECK"  # v29: low_pair 18.7%
                    if bt == "wet": return "BET"
                    if bt == "paired" and has_none: return "BET" if (4 < tr2 < 10) else "CHECK"
                if tier == "エア":
                    if has_strong:
                        if bt == "paired": return "BET" if tr <= 10 else "CHECK"
                        # v29: dry×ace_high×strong → CHECK (8.6%)
                        if bt == "dry" and is_ace_high: return "CHECK"
                        # v29: wet×ace_high×strong → CHECK (0.6%)
            ...
            if pot == "4BP":
                if tier == "TP+":
                    ...
                    if bt == "paired":
                        if has_gutshot: return "BET"
                        return "BET" if tr > 10 else "CHECK"
                    if bt == "dry" and mv == "overpair": return "CHECK"  # v29: overpair 34.7%
                ...
                if tier == "アンダーペア":
                    # v29: low_pair×dry×gutshot → CHECK (31.4%)
        ...
        if pos == "IP":
            ...
            if pot == "SRP":
                ...
                if tier == "エア":
                    if bt == "paired":
                        ...
                        # v29: ace_high×paired = 98.8% BET → BET (was incorrectly CHECK)
            ...
            if pot == "4BP":
                ...
                if tier == "エア" and bt == "paired" and is_ace_high: return "BET"  # v29: ace_high 91.5%
...
df["pred"] = df.apply(predict_v29, axis=1)
...
V28_LOSS = 26.6324; V28_AQS = 85.74
print(f"v29: acc={acc:.3f}%  avg_loss={avg_loss:.4f}%  AQS={score:.2f}")
print(f"vs v28: avg_loss {V28_LOSS:.4f}% -> {avg_loss:.4f}% ({avg_loss-V28_LOSS:+.4f}pp)  AQS {V28_AQS} -> {score:.2f} ({score-V28_AQS:+.2f})")
