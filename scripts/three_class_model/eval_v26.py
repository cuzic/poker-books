    ...
    if street == "flop":
        ...
        if pos == "IP":
            ...
            if pot == "SRP":
                ...
                if tier == "エア":
                    if bt == "paired":
                        # v23: king_high×paired → CHECK (6.1%)
...
df["pred"] = df.apply(predict_v26, axis=1)
...
V25_LOSS = 26.9398; V25_AQS = 84.61
print(f"v26: acc={acc:.3f}%  avg_loss={avg_loss:.4f}%  AQS={score:.2f}")
print(f"vs v25: avg_loss {V25_LOSS:.4f}% -> {avg_loss:.4f}% ({avg_loss-V25_LOSS:+.4f}pp)  AQS {V25_AQS} -> {score:.2f} ({score-V25_AQS:+.2f})")
