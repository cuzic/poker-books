    ...
    if street == "flop":
            ...
            if pot == "4BP":
                ...
                if tier == "エア":
                    ...
                    if bt == "paired":
                        ...
                        return "CHECK"
                    # v35: wet×ace_high×strong → BET (GTO=53.7%)
        ...
        if pot == "4BP":
            ...
            if tier == "エア":
                ...
                if bt == "wet":
                    ...
                    if is_king_high and has_strong: return "BET"
                    # v35: king_high×none → CHECK (GTO=40.3% on wet boards)
        ...
        if pot == "3BP":
            ...
            if tier == "エア":
                ...
                # v35: dry×king_high×strong → BET (53.5%)
        ...
        if pos == "IP":
            if tier == "2P+":
                ...
                # v35: 4BP×dry×straight×none → CHECK (GTO=35.6% — slowplay)
            ...
            if pot == "3BP":
                ...
                if tier == "エア":
                    if bt == "dry":
                        ...
                        # v35: gutshot×king_high → CHECK (22.3%)
                ...
                return "CHECK"
            if tier == "TP+":
                # v35: overpair×dry×none → BET (GTO=89%, tr2 8-12 was incorrectly CHECK)
            ...
            if pot == "3BP":
                if tier == "TP+":
                    ...
                    if bt == "paired" and has_gutshot: return "BET" if tr <= 5 else "CHECK"
                    # v35: paired×has_strong → CHECK (GTO=35.1% e.g. 4s4d2c5h top_pair oesd)
                ...
                if tier == "アンダーペア":
                    ...
                    # v33/v35: wet×third_pair×strong → CHECK (GTO=38.2%) — fix ordering vs wet→BET
                    if bt == "wet" and is_third_pair and has_strong: return "CHECK"
                    if bt == "wet": return "BET"
                    if bt == "paired" and has_none: return "BET" if (4 < tr2 < 10) else "CHECK"
            ...
            if pot == "4BP":
                if tier == "TP+":
                    ...
                    return "BET"
                if tier == "2P+":
                    # v35: wet×set×none → CHECK (GTO=39.3%)
        ...
        if pos == "IP":
            ...
            if pot == "4BP":
                if tier == "TP+": return "BET"
                if tier == "アンダーペア":
                    # v35: dry×underpair × none → BET (GTO=52.9%)
            ...
            if pot == "3BP":
                ...
                # v35: TP+ dry overpair → CHECK (GTO=22.4%, overpair traps on river)
                if tier == "TP+" and bt == "dry" and mv == "overpair" and has_none: return "CHECK"
                return "BET"
            if pot == "4BP":
                if tier == "TP+":
                    # v35: overpair×dry×none → CHECK (GTO=15.9%, trap)
...
df["pred"] = df.apply(predict_v35, axis=1)
...
V34_LOSS = 26.3822; V34_AQS = 86.66
print(f"v35: acc={acc:.3f}%  avg_loss={avg_loss:.4f}%  AQS={score:.2f}")
print(f"vs v33: avg_loss {V34_LOSS:.4f}% -> {avg_loss:.4f}% ({avg_loss-V34_LOSS:+.4f}pp)  AQS {V34_AQS} -> {score:.2f} ({score-V34_AQS:+.2f})")
