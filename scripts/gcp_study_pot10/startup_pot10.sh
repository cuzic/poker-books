#!/bin/bash
# VM startup script for pot10 GTO study.
# Downloads from GCS, runs worker_pot10.py, shuts down.
...
upload_log() {
    gsutil -q cp "$LOG" "gs://${GCS_BUCKET}/pot10_study/logs/vm${VM_INDEX}.log" 2>/dev/null || true
}
...
gsutil -q cp "gs://${GCS_BUCKET}/pot10_study/worker_pot10.py"          worker_pot10.py          && log "worker OK"
gsutil -q cp "gs://${GCS_BUCKET}/pot10_study/scenarios_pot10.json"     scenarios_pot10.json     && log "scenarios OK"
gsutil -q cp "gs://${GCS_BUCKET}/pot10_study/console_solver"           console_solver           && log "solver OK"
gsutil -q cp "gs://${GCS_BUCKET}/pot10_study/solver_resources.tar.gz"  solver_resources.tar.gz  && log "resources OK"
