...
スーテッドはフラッシュドローの価値。
Cash・MTT ともに同じ +{CASH['suit_bonus']} です。
...
T の値はシナリオ（ポジション・アクション・ゲーム種別）によって変わります。
係数は変えません。T だけを変えます。

## 2.2 2 段階閾値（コール vs 3-bet）

BB defense や IP/OOP defense では、
T_3bet（3-bet 基準）と T_call（コール基準）の 2 本になります。
...
の順になることを GTO データで確認済みです（全シナリオで成立）。
...
境界ハンドは GTO が混合戦略（例: raise 40%/fold 60%）を使います。
「どちらでもよい」の判断なので、本書では **T 以上をプレイ** と単純化します。

## 2.5 BB defense の補助ルール

BB defense は上記2段階閾値に加え、**補助ルール 1 個** を使います。
...
理由: BB は 1bb 投資済みでポットオッズが良く、スーテッドの implied odds が改善するためです。
...
つまり、この補正 1 個で **13 ポイントの改善** になります。
BB defense は本式の最難関シナリオで、補正なしでは
22 / 33 / 44 / 32s / 42s / 43s 等の「set mining 候補」を取りこぼしてしまいます。
"""
...
SB は OOP（ポジション不利）のため BTN より tight です。
ただしリンプも選択肢になります（GTO は混合戦略）。
本書では **raise or fold** に単純化します。
{_sb_line}

### SB RFI の整合率は低め（76%）

実測 GTO の SB RFI を本式と照合すると、整合率は **76%** です。
他のオープナー（UTG/HJ/CO/BTN）が 94〜97% なのに対して大きく低くなっています。
...
本書では Score < T_open は fold に単純化するため、これらの「広い open」を取りこぼします。
ただし実戦的には fold しても EV 損失は限定的です（混合戦略の境界帯）。

## 【GTO とのズレ】

**ズレが大きいハンド**: A2s〜A5s（GTO は 3-bet ブラフに使いますが、
本式では score が高いためオープン推奨になります。実際はどちらでも構いません）。

**ズレが小さいハンド**: スーテッドコネクター系（JTs, T9s, 98s…）は
gap=0 でギャップペナルティがなく、GTO と一致しやすいです。
...
ディフェンス判断はすべて同じスコア式を使います。閾値 T だけが変わります。
...
スラッシュなし行は T_3bet = T_call（コールレンジなし、3-bet or fold）です。
...
スラッシュ表記 = **T_3bet / T_call**。スラッシュなし = 3-bet or fold（コールレンジなし）です。
...
3-bet を受けてオープナーが判断する局面です（GTO データより）。
...
ポイント: 4-bet レンジ（Score ≥ {t4bet}）と 5-bet shove レンジは対称になります。
コールは ポットオッズ 28% + 残スタック SPR≈2.4 の implied odds を加味します。
...
**実戦推奨**: 相手の 5-bet レンジに KK や QQ が含まれると読めるなら KK・QQ・AKs はコールです。
AA のみと読めるなら KK もフォールドが理論的に正しい判断です。
...
本式では fold ですが GTO は 90〜100% call です。原因は set mining と BB ポットオッズです。
**第 2 章 §2.5 の suited −2 補正 + 低ペアコール推奨** で 74% → 87% に改善します。

**IP defense**: BTN vs CO は GTO が実際にはコールレンジを持ちますが、
本式では T_3bet ≈ T_call となり事実上 3-bet or fold に単純化されます。
それでも整合率 91.5% で本式が最もよく機能するシナリオです。

**SB OOP defense**: BTN open に対しては T_3bet = T_call = 25（コールレンジなし）が GTO です。
他オープナーに対しては 3-bet と call を分けます。

**vs 3-bet**: 12 シナリオすべてで 85〜93% の安定した精度があります。
T_4bet = 37 / T_call = 31 という 2 段階閾値が機能しています。

### 検証対象外

4.5（4-bet defense）/ 4.6（5-bet defense）は GTO ソルバー検証なしです。
ポットオッズ理論とモンテカルロ法（n=40,000）による近似値です。
"""
...
implied odds が成立すれば IP からコールドコールできます。
...
→ ペアが Cash より +{MTT['pair_bonus']-CASH['pair_bonus']} 強くなります。
...
SBR が低い（スタックが少ない）ほど：
- プッシュ/フォールドに近づきます
- ポストフロップの implied odds が消えます
- より tight なオープンレンジが最適になります

→ **SBR が低いほど T_open が高くなります**
...
1. 現在の実効スタックと BB を確認して SBR を計算します
2. 表から T_open を引きます
3. 自分のスコアと比較します

**例: 25BB スタック、BTN でハンドを受け取った**

T_open ≈ SBR=25 の値を使います。
...
目安として「最終卓付近では ±1〜2 の調整を行う」程度で大丈夫です。
...
1. 残り参加人数を把握します（ペイアウト人数の何%か）
2. 状況に応じて T_open を ±1〜2 調整します
3. 係数・スコア計算はそのままです

データが限られるため、GTO Wizard などで対象フェーズを直接確認することをお勧めします。
"""
...
H+L を素早く出してから、ボーナス/ペナルティを加算します。
...
閾値 T で判断が分かれるハンド一覧です。GTO が混合戦略を使う「グレーゾーン」です。
どちらに判断しても大きなエラーにはなりません。
...
スコア式でカバーできない約 7〜8% のケースを補完する 4 つのルールです。

## 12.1 小ペア set mining

ペアは式で自動的にスコアが高くなりますが、スタックが浅いと収益性が低下します。
T_call 未満のペアでも深スタックならコールが正当化されます。
...
低スコアの SC は式の T_call を下回ることが多いですが、
IP + 深スタックなら implied odds が成立します。
...
GTO は A スーテッド低カード・K9s・QJs などをブラフ 3-bet に使います。
本式ではスコアが高くオープン推奨になりますが、実戦では「ブラフ or open」どちらでも構いません。

主な 3-bet ブラフ候補: {', '.join(f'{h}({int(sc[h])})' for h in bluff_hands)}, K9s({int(sc['K9s'])}), QJs({int(sc['QJs'])})

本書では **式に従いオープン or フォールド** を推奨します。
上級者の方は T_3bet 付近のこれらのハンドで混合戦略を検討してみてください。

## 12.4 BB ワイドコール（suited 補正）

BB は 1BB 投資済みでポットオッズが改善します。
suited ハンドは T_call より広くコールできます。
...
精度 74% の主な誤分類はこの BB のワイドコールです。
suited 補正（−2〜3）を使うことで精度が約 90% に改善します。
