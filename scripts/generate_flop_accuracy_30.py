#!/usr/bin/env python3
"""巻4 検証 Phase 2: 30 ボード精度検証用シナリオ生成スクリプト.

`verify_flop_gto.py` の GTO_BOARD_DATA (30 ボード) から、レンジ全体
(BtnOpen100bb vs BbDefendVsBtn) のフロップシナリオ JSON を生成する。
poker-gto 側の `solve_from_json` CLI 経由で並列実行し、Bet 頻度を
GTO Wizard 公開値と比較するのが目的。

出力先: /home/cuzic/poker-books/knowledges/volume4/scenarios/flop_accuracy_30/
...
# ---------------------------------------------------------------------------
# ボードラベル → 具体カード 3 枚 (generate_volume4_102.py と同じ割付)
# ---------------------------------------------------------------------------
...
def cards_for_label(label: str) -> List[str]:
    ...
    割付ルール (generate_volume4_102.py と同一):
      - mono: すべて "s"
      - ss:   上位 2 枚が "s"、最下位が "h"
      - r:    c/d/s の 3 スート
...
def build_scenario(board_label: str, cards: List[str], gto_freq: int) -> Dict[str, Any]:
    """1 ボード分のシナリオ dict を構築する.

    設定:
      - street_to_solve: "flop"
      - hero_range: BtnOpen100bb (レンジ全体)
      - villain_range: BbDefendVsBtn
      - pot_bb: 7 (BTN 2.5x + BB call の SRP)
      - effective_stack_bb: 97
      - hero_position: "BTN" (IP)
      - bet_sizes_to_evaluate: [0.33, 0.5, 0.75, 1.5]
      - solver_config: ES_MCCFR, iterations=5000, timeout=60s, seed=42
    """
    scenario_id = f"flop_acc30_{board_label}"
    description = f"{board_label} フロップ精度検証 (GTO Wizard {gto_freq}%)"
...
def validate_scenario(scenario: Dict[str, Any], schema: Optional[Dict[str, Any]]) -> None:
    if schema is None:
        return
    try:
...
def main() -> None:
    ...
    index_payload = {
        "task": "flop_accuracy_30",
        "title": "30 ボード精度検証 (Range API 実装後)",
