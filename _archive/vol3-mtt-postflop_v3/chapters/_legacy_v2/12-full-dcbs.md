# 第12章 D モデル——4 context の守備 continue freq

D モデル (Defense TV) は BB の cbet に対する continue freq を MV × context で予測するモデルです。
A モデル (攻撃、本書 ch01-11) と D モデル (守備、本章) は独立したモデルで、相互に影響しません。
...
## D モデルの構造——MV バケット × context の 2 段テーブル

D モデルの計算式はシンプルです。

```
continue_freq = base_dcbs[context][MV]
              + kicker_offset[context][hand]  (MV=2 の場合のみ)
fold_freq = 1.0 - continue_freq
```

A モデルが TV (MV+DV)、α/β/cat/ε など複数の layer を使うのに対し、
D モデルは MV のみで base を引き、MV=2 (air 系) に限り手別の kicker_offset で細分化します。

攻撃 (A モデル) と守備 (D モデル) が別モデルである理由は、defense の判断基準が攻撃と異なるためです。
defense は「相手の bet に対してコールかフォールドか」を決める問題であり、
ボードとの相性よりも「自分の手の絶対的な強さ (MV)」と
「スタック深度 (context)」が支配的です。
DV (ドロー点) は defense でも考慮されますが、D モデルでは単純化のため MV のみを使います。
...
D モデルの最重要発見は「スタック深度が深いほど air を fold する」という反転構造です。

MV=2（air 系: no_made_hand / ace_high / king_high / low_pair）の base continue freq:
...
一方、top_pair 以上（MV=7 以上）は全 context で 96%+ のコール頻度を示します。
...
MV=3（弱ペア: underpair / third_pair）でも 84% とやや低く、
MV=5（second_pair）で 87% とスタックが深いほど middle ハンドも慎重になります。
...
## Kicker offset——MV=2 内の ace_high / king_high 細分化

MV=2 の中には no_made_hand（ノーペア）、ace_high（Aハイ）、king_high（Kハイ）、
low_pair（ローペア）の 4 種類が含まれます。
これらを MV のみで扱うと「Aハイも ノーペアも同じコール頻度」となり、
GTO データとの乖離が生じます。
kicker_offset はこの MV=2 内の細分化を担います。
...
## D モデルの実戦運用——5 秒で continue を決める

実戦での D モデル 運用フローは以下の通りです。

**Step 1**: 自分の手の MV を確認する。
- MV=2: no_made_hand / ace_high / king_high / low_pair
- MV=3: underpair / third_pair
- MV=5: second_pair
- MV=7: top_pair / overpair
- MV=8+: set / trips / two_pair / flush / straight 以上
...
**Step 3**: base 表を引く。MV=2 なら kicker_offset を加算する。

**Step 4**: 算出した continue_freq が 50% 以上ならコール、未満ならフォールド。
（ただしレイズオプションがある場合はチェックレイズを別途検討する。）

最も重要な判断ポイントは MV=2（air 系）の context 別分岐です。
25bb ならば「3 回に 2 回はコール」、100bb ならば「4 回に 1 回しかコールしない」という
直感的なルールを覚えておくと実戦で即座に判断できます。

## D モデル vs MDF——GTO 根拠の確認

MDF（Minimum Defense Frequency）は pot bet のサイズから算出される最低守備頻度です。
相手が pot の 33% をベットした場合：MDF = ポット ÷ (ポット + bet) = 1 ÷ (1 + 0.33) ≈ 75%

MDF は全体レンジの守備頻度を示しますが、D モデルは個々の手の continue freq を示します。
D モデルの全体的な continue freq がおおよそ MDF と整合しているかを確認することで、
レンジ全体のバランスを確認できます。

たとえば mtt_50bb では MV=2 が 54%（範囲内の手の大半）、MV=3 が 95%、MV=5+ が 96%+ であり、
加重平均すると全体の continue freq は概ね 70〜75% 程度となり、MDF と整合しています。
D モデルは MDF の考え方を MV 別に細分化したモデルと理解できます。

## 実戦例題

### D モデル continue freq 計算例（4 context）
...
- **D モデル 式**: continue_freq = base[MV] + kicker_offset[hand]（MV=2 のみ kicker 適用）
- **depth 反転の核心**: air(MV=2)は浅い(25bb)=67% → 深い(100bb)=28%（2.4倍の差）
- **top_pair 以上は depth 不問**: 全 context で 98%+ のコール
- **kicker 効果は深さで减衰**: 浅(ace vs no_made 差 22pt) → 深(5pt)
- **A モデルとは別モデル**: 攻撃と守備は設計が異なる（MV のみ使用）
- **MDF 整合**: D モデルの加重平均 ≈ 33% bet に対する MDF 75% 程度と整合
