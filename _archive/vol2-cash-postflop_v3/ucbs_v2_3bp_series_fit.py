#!/usr/bin/env python3
"""UCBS-v2 3BP IP series fit (25/50/100 bb depths)"""
        ...
        for line in f:
            ...
            for h, vals in entry.get("hand_agg", {}).items():
                if h not in HP_TABLE or vals.get("total", 0) < 3:
                    continue
                out.append({
                    "scenario": "BTN", "board": board, "bt_str": bt_str, "bt": bt,
                    "hand": h, "n": vals["total"],
...
def main():
    files = {
        "3bp_20bb": "/home/cuzic/poker-books/mtt-postflop/findings/draw_study_3BP20.jsonl",
        "3bp_25bb": "/home/cuzic/poker-books/mtt-postflop/findings/draw_study_3BP25.jsonl",
        "3bp_50bb": "/home/cuzic/poker-books/mtt-postflop/findings/draw_study_3BP50.jsonl",
        "3bp_100bb": "/home/cuzic/poker-books/mtt-postflop/findings/draw_study_3BP100.jsonl",
    }
    print(f"{'depth':12s}  {'records':>7s}  {'combos':>7s}  "
    ...
    for label, fp in files.items():
        ...
        print(f"{label:12s}  {len(recs):>7d}  {int(total_n):>7d}  "
    ...
    # Joint fit: 4 depths をプールして "ベースライン + depth lift"
    print("\n[Hand 別 bias by depth]")
    print(f"  {'hand':14s}  " + "  ".join(f"{d:>10s}" for d in fits.keys()))
    for h in ["no_made_hand", "ace_high", "king_high", "low_pair", "underpair",
              "third_pair", "second_pair", "top_pair", "overpair",
              "two_pair", "straight", "set", "trips", "fullhouse"]:
        row = [f"  {h:14s}"]
        for label, theta in fits.items():
            recs_h = [r for r in all_records if r["hand"] == h and r["depth"] == label]
            if not recs_h:
                row.append(f"{'—':>10s}")
                continue
            esum = sum(r["n"] * (predict(r,
                alpha=theta[0], beta=theta[1],
                off_slowplay=theta[2], off_trash=theta[3], off_premium=theta[4]
            ) - r["gto"]) for r in recs_h)
            n = sum(r["n"] for r in recs_h)
            if n > 0:
                row.append(f"{esum/n*100:>+8.1f}%")
            else:
                row.append(f"{'—':>10s}")
        print("  ".join(row))
