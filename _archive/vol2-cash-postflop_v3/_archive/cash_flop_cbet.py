#!/usr/bin/env python3
"""
cash_flop_cbet.py — フロップCBet（IP）＆OOP守備 分析
...
使い方:
  TOKEN=eyJ... GWCLIENTID=xxx python3 cash_flop_cbet.py
  TOKEN=eyJ... SCENARIO=CO_BB python3 cash_flop_cbet.py
  TOKEN=eyJ... TYPE=型1 python3 cash_flop_cbet.py
"""
...
def analyze_board(cfg, scen):
    ...
    cr_code    = dominant_bet_code(oop_codes)  # CR の主要サイズ
...
def main():
    ...
    # JSON 保存
    out = FINDINGS_DIR / f"flop_cbet_{SCENARIO}.json"
    with open(out, "w", encoding="utf-8") as f:
        json.dump(
            {"scenario": scen["label"], "results": all_results},
