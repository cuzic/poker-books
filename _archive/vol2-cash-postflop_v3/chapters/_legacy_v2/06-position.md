...
A モデルでは pos_lift (ポジション補正) を 4 値で管理します。
...
A モデルではこの有利不利を **pos_lift** (ポジション補正) という数値で定量化しました。
...
| ポジション | pos_lift | 理由 |
|---|---:|---|
| BTN | 0pt | 基準。A モデルの 25 セル表がそのまま適用 |
...
A モデルではこの効果を数値化せず、Vol3 の上級トピックに委ねます。
