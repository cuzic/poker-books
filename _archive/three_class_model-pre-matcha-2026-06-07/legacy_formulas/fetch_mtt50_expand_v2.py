...
# ///
"""MTT 50bb expansion v2 — slower sleep, retry remaining boards."""
...
# Boards that were rate-limited
TURN_BOARDS_REMAINING = [
    ("Ts9d8cKs", "T98-K"),
    ("Ts9d8c5d", "T98-5"),
    ("Ts9d8cQc", "T98-Q"),
    ("Ts9d8c6h", "T98-6"),
...
def main() -> int:
    ...
    with httpx.Client(headers=HEADERS) as client:
        # Turn defense remaining
        print("=== Turn defense (remaining) ===", flush=True)
        for board, label in TURN_BOARDS_REMAINING:
            existing = list(OUT_T.glob(f"{label}_*.json"))
            if existing:
                print(f"  SKIP {label} (has {len(existing)} file)", flush=True)
                continue
            params = dict(BASE)
            ...
            n_total += 1
            if st != 200 or not d:
                print(f"  PROBE {label}: {st}", flush=True); time.sleep(SLEEP); continue
            bet_actions = [a for a in d.get("action_solutions", []) if a.get("action", {}).get("type") in {"RAISE", "BET"}]
            time.sleep(SLEEP)
            if not bet_actions:
                print(f"  {label}: no bet", flush=True); continue
            for ba in bet_actions:
                ...
                if st == 200 and d2:
                    ...
                    n_ok += 1
                    print(f"  ✓ {label} bet={bp_str}({code})", flush=True)
                time.sleep(SLEEP)

        # River defense remaining
        print("\n=== River defense (remaining) ===", flush=True)
        for board, label in RIVER_BOARDS_REMAINING:
            ...
            if not chosen_turn or not d:
                print(f"  PROBE {label}: no valid", flush=True); continue
            bet_actions = [a for a in d.get("action_solutions", []) if a.get("action", {}).get("type") in {"RAISE", "BET"}]
            time.sleep(SLEEP)
            if not bet_actions:
                print(f"  {label}: no bet", flush=True); continue
            for ba in bet_actions:
                ...
                if st == 200 and d2:
                    ...
                    fn.write_text(json.dumps(d2))
                    n_ok += 1
                    print(f"  ✓ {label} river={bp_str}({code})", flush=True)
                time.sleep(SLEEP)

    print(f"\nTotal: {n_total}, saved: {n_ok}, elapsed: {time.time()-started:.1f}s", flush=True)
