...
そこで本書は **一桁大きい +10** で差別化します。77（スコア24）がT_open(UTG)にちょうど収まるように、値を逆算した形でもあります。
...
たとえば AJo（差3、ギャップ2）は 14 + 11 + 0.5 + 3 (Aブロッカー) = **28.5**。端数の 0.5 が、しきい値周辺で小さな差を生むことがあります。
...
差が4以上（K8、Q7、J6など）になると、ストレートを作れる並びが激減します。
...
本章で示したボーナス・ペナルティ値はすべて**丸められた近似**です。実際のEV差は連続値で、状況によっても変動します。たとえばスーテッドの価値は、「同じマークのコネクター」と「広くギャップのあるスーテッド」では異なります。

GTOソルバーはこれらを連続的に評価しますが、**暗算では連続値を扱えません**。本書が整数（または0.5刻み）に丸める理由はここにあります（本書全体の設計方針は第1章で宣言済みです）。
...
- **高いカード同士は少し上積み**：JTs や KQs は +1 するイメージ（ブロードウェイ価値）
- **低いカード同士は少し割引**：65s や 54s は −1 するイメージ（ナッツ可能性が高くてもキッカー負けが多い）
- **マルチウェイ対応**：マルチウェイ局面ではオフスート BW（KJo / QJo 等）は −2、小ペアは +2 で調整（第14章）

ブロッカー効果はすでにスコア式に組み込まれている（A=+3、K=+2、AK=+4）ため、A5s や AJo に対する追加補正は不要です。本書の式で違和感が出たら、上記 3 つの軸で微調整できます。
