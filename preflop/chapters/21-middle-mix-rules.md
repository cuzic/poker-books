...
Tは既存のしきい値（UTG=24, MP=22, CO=21, BTN=18, SB=22）をそのまま使います。
...
たとえばUTGでスコア23のハンドは、旧ルールでは「24未満なのでフォールド」と一律に判断されていました。R1のもとでは「混合ゾーン」に入るため、次節のR2で最終判断を下します。
...
COの3バンド：弱バンド≤18、混合ゾーン19〜22、強バンド≥23。
...
UTGで77（スコア24）を持っています。混合ゾーン（22〜26）の中央値です。R1適用後、7のスートが♣（クラブ）でした。R2を適用するとどうなりますか。

**しきい値**：UTG = 24

**解答**

UTGの混合ゾーンは22〜26。スコア24は混合ゾーンです。

高い方のカードである7（ペアなので片方）のスートが♣。これは受動側（♣♦）なので**フォールドです**。

77は旧ルールでは「UTGしきい値24にちょうど一致でオープン」でしたが、R1適用後は混合ゾーンに入り、スートによってフォールドすることがあります。これが「境界を揺らす」効果です。
...
対UTGしきい値23の3バンド：弱バンド≤20、混合ゾーン21〜24、強バンド≥25。（3betしきい値は据え置き）
