...
Tは既存のしきい値（UTG=23, MP=22, CO=20, BTN=18, SB=22）をそのまま使います。
...
たとえばUTGでスコア22のハンドは、旧ルールでは「23未満なのでフォールド」と一律に判断されていました。R1のもとでは「混合ゾーン」に入るため、次節のR2で最終判断を下します。
...
UTGのしきい値23。混合ゾーンは21〜25。スコア23.5は混合ゾーンに入ります。
...
UTGで77（スコア24）を持っています。混合ゾーン（21〜24）の上限値です。R1適用後、7のスートが♣（クラブ）でした。R2を適用するとどうなりますか。

**しきい値**：UTG = 23

**解答**

UTGの混合ゾーンは21〜24。スコア24は混合ゾーンです。

高い方のカードである7（ペアなので片方）のスートが♣。これは受動側（♣♦）なので**フォールドです**。

77は旧ルールでは「UTGの23を1点上回るのでオープン」でしたが、R1適用後は混合ゾーンに入り、スートによってフォールドすることがあります。これが「境界を揺らす」効果です。
...
UTGでA8o（スコア22.5）を持っています。R1を適用した場合、何バンドに入りますか（UTGしきい値23）。
