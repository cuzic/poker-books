...
T_3bet=32 にも T_call=28 にも届かないので**フォールド**。BTN でもフォールド推奨。「強そうなのに BTN でフォールド」は意外ですが、UTG オープンレンジ（QQ+/AK 中心）に対して QJo はドミネートされる側です。CO/HJ オープン（より広いレンジ）に対しては別判断（第14章 閾値補正）。
