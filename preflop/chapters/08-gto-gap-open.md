...
本書の「スーテッド +3」は、フラッシュドロー（11%）＋エクイティ向上（3〜5%）＋プレイアビリティを反映した値です。しかしブロッカー効果や3ベット耐性までは評価できていません。実際、A5sの本書スコアは14 + 5 + 3 − 1 = 21で、UTGしきい値23を下回ります。GTOはUTGでA5sを開く一方、本書はフォールドと判定します。
...
本書のしきい値はレーキなしを前提にしています。NL25以下の高レーキ環境では、**しきい値を1点厳しめに設定**するとよいでしょう。たとえばUTG 23 → 24に上げると、境界ハンドの参加がさらに減ります。
...
| QJs | UTG オープン（27） | HJ〜CO | スーテッド+コネクター二重加算 |
