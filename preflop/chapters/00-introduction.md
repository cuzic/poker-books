...
### Q4. GTOソルバー（GTO Wizard 等）でプリフロップチャートを週 1 回以上確認していますか？

- はい → Q5へ
- いいえ → **Level 2 から開始**（第21章「固定を揺らす」）

### Q5. MTT やサテライトで ICM を考慮してプレイしていますか？

- はい → **Level 3 のみ**で十分（第17章ICM拡張、第21章）
- いいえ → **Level 2 + Level 3 の両方**を学習してください
...
### Level 3：上級・応用（第17章 ICM、第21章完全適用）

**対象**：Level 2をマスター、MTTもプレイしソルバーを併用する人。

**到達目標**：

- マルチウェイ・ICM補正を式に組み込める
- MTTバブル前後のしきい値調整ができる
- 一連の判断を5秒以内に実行できる
- 約500NL以上・MTT中盤〜ファイナルテーブルで通用する基盤を持つ
...
第0章（用語）→ 第1章（動機づけ）→ 第2章（数値化）→ 第3〜4章（式の発想）→ 第5〜7章（基本式）→ 第8章（GTOとのズレ）→ 第9〜12章（対応判断）→ 第13〜16章（応用）→ 第17章 → 第18〜20章（定着）→ 第21章。

### 実戦重視の読み方（短縮）

第5章（基本式）→ 第7章（しきい値）→ 第19章（チートシート）→ 第12章（3bet）→ 第21章 → 第17章（マルチウェイ・ICM）。
