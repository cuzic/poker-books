...
**フロップ以降（ポストフロップ）は扱いません。** フロップ・ターン・リバーの戦略は姉妹編『迷わないポーカー② フロップ[基礎]』を参照してください。
...
### Q2. ポジション名（UTG、HJ、CO、BTN、SB、BB）を即答できますか？
...
- 姉妹編『迷わないポーカー② フロップ[基礎]』の Board Score B / HandScore / T1/T2/T3 ティア を習得
- フロップ段階での判断を式で決定できる
- 約500NL以上で通用する基盤を持つ
...
ポットオッズ、実現率、セットマイニング、プリフロップスコア + 二段閾値による 3-bet 判定、GTO との乖離点を扱います。
