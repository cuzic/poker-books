...
<figure><img src="images/p02-fig1-6max-table-positions.jpg" alt="6-max テーブルの俯瞰図に UTG/HJ/CO/BTN/SB/BB の各ポジションをラベルで示す図" /><figcaption>6-max テーブルの俯瞰図に UTG/HJ/CO/BTN/SB/BB の各ポジションをラベルで示す図</figcaption></figure>

<figure><img src="images/p02-fig2-position-value-bars.jpg" alt="UTG 24 / HJ 22 / CO 20 / BTN 18 / SB 22 のポジション別しきい値を横棒グラフで視覚化" /><figcaption>UTG 24 / HJ 22 / CO 20 / BTN 18 / SB 22 のポジション別しきい値を横棒グラフで視覚化</figcaption></figure>
...
|  HJ | Hijack（Middle Position） | 2番目に行動 |
| CO | Cutoff | BTN直前 |
| BTN | Button | ディーラーボタン（最後に行動） |
| SB | Small Blind | スモールブラインド |
| BB | Big Blind | ビッグブラインド |

※ 6-maxのMPは「HJ（Hijack、ハイジャック）」と呼ばれることもあります。本書ではHJで統一します。
...
- **HJ（Hijack / MP）**：HJは「ハイジャック」の略。COとBTNより先に動いて、盗む（ハイジャックする）席の意味
...
「Score を補正する」ではなく「**しきい値が違う**」と覚えます。これは式の構造を単純に保つための設計判断です。例えば AKo の Score は 28 で固定、UTG ではしきい値 24 を超えるので開く、BTN でも 18 を超えるので開く。Score は変わりません。
...
SB の RFI は表面上 39〜47% に見えますが、これは「raise + limp」の合計です。raise 部分のみ取ると約 24.3%、 HJ 並みのタイトさになります。
...
本書のしきい値（UTG 24 / HJ 22 / CO 20 / BTN 18 / SB 22）は、GTO の RFI 頻度に合わせて校正した値です。本書は初心者向けに**全ポジションで GTO よりわずかにタイト**になるよう設計されており、GTO 的に厳密であろうとするなら境界付近の扱いに細かい差が出ます。
...
- **UTG しきい値 24 は GTO 17% よりわずかにタイト**：本書 Score 式での該当ハンド数は約 14.2%。77 はぎりぎり残るが、A2s-A7s や T9s/98s/87s/76s は Level 1 では落ちる（Level 2 で救済）
- **CO しきい値 20 は GTO 27.8% よりわずかにタイト**：本書では約 25.2%。A2s 以下（HJ以降）、K2s 以下、Q5s 以下、J7s/J6s 等が Level 1 では落ちる
...
- 6-maxのポジションはUTG、HJ、CO、BTN、SB、BBの6つ
- BTN vs BBのエクイティ差は57.4% vs 42.6% で、ポジションの価値は圧倒的
- IP側の実現率は118%超、OOP側は79%程度（BTN vs BBの一例）
- 本書のしきい値は UTG 24 / HJ 22 / CO 20 / BTN 18 / SB 22（Score がこの値以上なら open）
...
1. UTG と BTN では T_open が 6 点違う（24 と 18）
...
- **問2**：1（UTG 24 と BTN 18 は 6 点差。Score 自体は補正しない、しきい値だけが違う。SB は混合戦略の raise 部分のみ抽出するため、しきい値 22 と比較的タイト）
