...
- **インターフェースは基本英語**（一部UI要素の日本語表示あり）

本書を読み終えた方が最初に触るべきツールです。

### 無料版でできること（2025年時点）

- 一部ポジションのプリフロップチャート閲覧（制限あり。全ポジション全スタックの閲覧には有料プランが必要）
- デイリートレーナー（1日1問、プリフロップ判定の練習）
- プリフロップレポート（機能限定版）
- 10BB以下のPush/Fold基本チャート

無料版でも本書の内容をGTO視点で確認する用途には活用できます。詳細な機能制限は GTO Wizard 公式サイト（英語）でご確認ください。
...
### Step 4：エクスプロイト戦略の応用（O'Kearney & Carter）

- **目的**：エクスプロイト戦略の応用
- **特徴**：GTOの上に「相手の弱点を突く」層を重ねる
- **本書との関係**：第14章の「相手タイプ別調整」の完全版

O'Kearney & Carterの『GTO Poker Simplified』（2022）の続編として計画・出版されているシリーズ。最新タイトルと出版年は刊行時点でご確認ください。GTOを学んだ後、実戦でさらに勝率を上げるための最終段階。
