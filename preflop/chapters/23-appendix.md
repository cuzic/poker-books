...
⑤ **R6：相手タイプで MDF を補正**（相手VPIP/PFRを見る）
...
#### 5 補正適用（Level 2）で 94.6%、例外表（Level 3）で 100% 一致

第22章の「5つの名前付き補正」を適用したLevel 2では、オープン判定のGTO一致率が **799/845 = 94.6%** に達します。UTGは5補正だけで100% 一致となります。

**Level 2（5 補正）オープン判定の一致率**
...
5補正は「基本スコア式の書き換えなし・名前付きカテゴリルールの上乗せ」というアプローチで実現しています。実用上は5補正（94.6%）で十分で、例外表は「参照する」感覚で使います。詳細は第22章を参照してください。
