...
| ハンド | スコア | 計算 | 開けるポジション |
|--------|-------|------|----------------|
| AKs | 31 | 14+13+3+1 | 全ポジション |
| AQs | 29.5 | 14+12+3+0.5 | 全ポジション |
| AJs | 28.5 | 14+11+3+0.5 | 全ポジション |
| ATs | 27 | 14+10+3 | 全ポジション |
| A9s | 25 | 14+9+3−1 | UTG〜 |
| A8s | 24 | 14+8+3−1 | UTG〜 |
| A7s | 23 | 14+7+3−1 | MP〜 |
| A6s | 22 | 14+6+3−1 | MP〜 |
...
| ハンド | スコア | 計算 | 開けるポジション |
|--------|-------|------|----------------|
| KQs | 29 | 13+12+3+1 | 全ポジション |
| KJs | 27.5 | 13+11+3+0.5 | 全ポジション |
| KTs | 26.5 | 13+10+3+0.5 | 全ポジション |
| K9s | 25 | 13+9+3 | UTG〜 |
| K8s | 23 | 13+8+3−1 | MP〜 |
| K7s | 22 | 13+7+3−1 | MP〜 |
| K6s | 21 | 13+6+3−1 | CO〜 |
...
| J9s | 23.5 | 11+9+3+0.5 | MP〜 |
| T9s | 23 | 10+9+3+1 | MP〜 |
| 98s | 21 | 9+8+3+1 | CO〜 |
| 87s | 18 | 8+7+3+1-1 | BTN |
| 76s | 16 | 7+6+3+1-1 | BTN（補助） |
| 65s | 14 | 6+5+3+1-1 | BTN（補助） |
| 54s | 12 | 5+4+3+1-1 | BTN（補助） |
...
| KTo | 23.5 | 13+10+0.5 | MP〜 |
| QJo | 24 | 12+11+1 | UTG〜（境界） |
| JTo | 22 | 11+10+1 | MP〜 |
| 72o | 7 | 7+2-1-1 | 全ポジフォールド |

<figure><img src="images/range-table-score-heatmap.png" alt="169ハンドのスコアヒートマップ。高スコア（緑）ほど広いポジションでオープン可能。しきい値ライン UTG≥24 / MP≥22 / CO≥21 / BTN≥18 / SB≥22 付き" /><figcaption>付録B 補足：ハンドスコア ヒートマップ — 各セルに手名とスコアを表示。グラデーションバーの縦線がポジション別しきい値</figcaption></figure>
...
スコア式はH + L（= 14 + 小カード）の合計が低くなるうえ、ギャップ4以上（差5以上）ペナルティが効いて評価が下がります。しかしGTOは次の3要素を高く評価します。
...
| 項目 | 内容 |
|------|------|
| 代表ハンド | QJs（スコア27）、KQo（スコア26） |
| 本書スコア | QJs=27（12+11+3+1）、KQo=26（13+12+1） |
| 本書の判定 | UTGしきい値24を超えるため**オープン** |
| GTO推奨 | **HJ 以降が標準** |

#### ズレの理由

QJsはスーテッド +3とコネクター +1の二重ボーナスで高スコアになります。KQoはコネクター +1で境界を超えます。しかしGTOのUTGレンジ（約18%）にはどちらも含まれないか、ごく限定的な混合でしか採用されません。
...
第15章で扱った通り、SBは基本「3ベットかフォールド」です。実用 GTO（poker-coaching チャート実装形）では cold-call 頻度が 0% で、AJs・KQs・KJs もすべて 3 ベットかフォールドに振り分けられます。理論上の混合戦略では「3ベットレンジを強くしすぎない」目的でこれらを一部コールに残す解も存在しますが、頻度は実データで 0.14% 程度（GTOBase）と無視できる水準です。

本書は実用 GTO に合わせ、SB の cold-call を採用しません。

#### 暗記ポイント

**「SB は 3ベットかフォールドのみ。AJs・KQs・KJs もコールしない」**。混合戦略を取り入れたい上級者は、AJs・KQs などをわずかにコール枠へ振る選択肢もありますが、初心者のうちは単純に**「SBは全部3ベットかフォールド」**で構いません。
...
R3適用：21 + 0 = 21。対CO 3bet T = 19の強バンド（≥ 21）。**3ベット確定**。
...
| 本書記載箇所 | 引用値 | ツール / 条件 |
|------------|-------|-------------|
| 第5章 5-2節 | UTG 24 / MP 22 / CO 21、CO 約 25〜28%、BTN 約 40〜43% | GTO Wizard 6-max 100BB Cash、各ポジションのオープンレンジ分析 |
...
| ポジション | しきい値 | コンボ加重一致率 | ハンド一致率 | FP | FN |
|-----------|---------|--------------|-----------|----|----|
| UTG | 24 | **97.1%** | 94.7% | 0 | 9 |
| MP | 22 | **94.1%** | 92.3% | 4 | 9 |
| CO | 21 | **91.9%** | 89.3% | 3 | 15 |
