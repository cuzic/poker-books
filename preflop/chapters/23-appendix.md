...
|-----------|---------|-------|-----|-------|----|----|
| UTG | 24 | **88.8%** | 86.2% | 62.5% | 4 | 15 |
| MP | 22 | **86.4%** | 79.1% | 70.8% | 9 | 14 |
| CO | 21 | **87.0%** | 86.3% | 74.6% | 7 | 15 |
| BTN | 18 | **85.2%** | 87.9% | 83.0% | 10 | 15 |
| SB | 22 | **90.5%** | 95.3% | 74.5% | 2 | 14 |

- **一致率**：169ハンドのうち、本書の判定がGTOレンジと一致した割合
- **FP**（False Positive）：本書は「オープン」、GTOは「オープンしない」
- **FN**（False Negative）：本書は「フォールド」、GTOは「オープンする」

全体集計：**ハンド一致率 約 87.6%、コンボ加重 約 90.6%（ポジション別平均）**。本書はわずかにタイト寄りに設計されているため、GTO が境界で開く一部ハンド（A2s-A6s、98s-76s 等）は Level 1 では落ちます（Level 2 で救済）。
...
- **コンボ加重 約 90.6%** は、暗算式として高い精度です
- 低スーテッドの裾野拾いが不足するため、本書の式に加えて**補助ルール**（BTNでK2s〜K9sはすべてオープン、など）を知っていると一致率が93%+ まで上がります
...
#### 5 ルール適用（Level 2）で 約 93%、例外表（Level 3）で 約 95〜97%

第22章の「5つの名前付きカテゴリ例外ルール」を適用した Level 2 では、オープン判定の GTO 一致率がコンボ加重で **約 93%** に達します。さらにポジション別例外表（Level 3）を取り入れると **約 95〜97%** まで届きます。

5ルールは「基本スコア式の書き換えなし・カテゴリ的例外の上乗せ」というアプローチで実現しています。実用上は5ルール（約 93%）で十分で、例外表は「参照する」感覚で使います。詳細は第22章を参照してください。
