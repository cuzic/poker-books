...
ここまでの章で、本書のしきい値は固定でした。UTGは24、BTNは18、SBは20——いつでも同じです。しかし実戦では、相手や状況によってこの数字を**動かす**べき場面があります。
...
> **補足（第21章への接続）**: 本章の±2調整は相手タイプとスタック深度の2軸による粗い近似ですが、第21章のR6（後手下限確率±10%）と組み合わせると、相手のプレイスタイルに応じたディフェンス頻度も同じ枠組みで扱えるようになります。
