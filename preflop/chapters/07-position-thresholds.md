...
|-----------|-----------------|-----------|---|
| UTG | 約 14% | 約 17.0% | **−3%（やや厳しめ）** |
| MP | 約 22% | 約 21.4% | ほぼ同等 |
| CO | 約 25% | 約 27.8% | **−3%（やや厳しめ）** |
| BTN | 約 44% | 約 43.3% | ほぼ同等 |
| SB | 約 22% | 約 24.3%（raise） | **−2%（やや厳しめ）** |

**初心者向けに、全ポジションで GTO と同じか少しタイトめになるよう設計**しています。やや開けない境界ハンドが出ますが、本書の「迷ったら降ろす」方針と整合し、ポストフロップの難しい判断を避けられます。GTO に厳密に寄せたい場合は第 22 章「カテゴリ例外ルール」で補正します。
...
しきい値の具体的な数値は、次の3ステップで設計しました。

**ステップ1：ポジションごとのアンカーペアを決める**

各ポジションの「最小オープンペア」をGTOレンジに合わせて設定します。GTO Wizard 6-max 100bb のデータでは、UTGは77以上がオープン対象（66は混合）、MPは66以上、BTNは44以上です。このアンカーペアのスコア（ペア値 × 2 + ペアボーナス10）がそのまましきい値になります。

| ポジション | アンカーペア | スコア計算 | → しきい値 |
|-----------|------------|-----------|-----------|
| UTG | 77 | 7 + 7 + 10 = 24 | **24** |
| MP | 66 | 6 + 6 + 10 = 22 | **22** |
| CO | 98s | 9 + 8 + 3 + 1 = 21 | **21** |
| BTN | 44 | 4 + 4 + 10 = 18 | **18** |
| SB | — | MPと同等（後述） | **22** |

**ステップ2：GTO RFI との整合を確認する**

上表のしきい値でオープンされるコンボ数をスコア式で全数列挙し、GTO Wizard のRFI%と比較します（上の「GTOレンジとの整合性」表）。差が±3%以内に収まることをもって整合性を確認しています。

**ステップ3：SBだけ特別処理する**

SB の GTO 戦略は「レイズ + リンプ」の混合です。本書は「レイズかフォールドか」の二択に簡略化するため、レイズレンジのみに相当する MP 並みの 22 を採用しています。
...
77はUTG開きの下限ハンドとして設計された基準点です。スコアちょうど 24 でオープン側に振ります。66以下のペアはMP以降で扱います。
...
- 77はUTG下限ペア（スコア24）、66はMP下限ペア（スコア22）、44はBTN下限ペア（スコア18）
