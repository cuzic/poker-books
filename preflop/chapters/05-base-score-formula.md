...
<figure><img src="images/p05-fig4-threshold-ladder.jpg" alt="UTG 24 / MP 22 / CO 21 / BTN 18 / SB 22 のポジション別プレイしきい値ラダー" /><figcaption>UTG 24 / MP 22 / CO 21 / BTN 18 / SB 22 のポジション別プレイしきい値ラダー</figcaption></figure>
...
しきい値は GTO の RFI 頻度（UTG 約 17% 〜 BTN 約 43%）に Score 式の値域を合わせて校正しました。UTG は最もタイト、BTN は最もルース。**初心者向けに全ポジションで GTO よりわずかに（0〜4 ポイント）タイトになるよう設定**しています。**SB は表面上 RFI が広く見えますが、これは raise + limp の混合戦略の合計です。本書は「raise か fold」のシンプル戦略を採るため、SB のしきい値は raise レンジ（約 24%）に合わせて 22 と MP 並みに設定しています**。
...
**UTGしきい値24にちょうど一致**。UTGでもオープンできる最も低いペアです。境界のハンドは設計上の基準点になっています。
...
UTGしきい値24を1点下回る。**MP以降でオープン**。境界の好例：T9sはスーテッド+コネクターで強く見えますが、本書ではUTG では1点足りずフォールドに振ります。
...
| 22〜23 | 66、A7s、T9s、KTo、A6s | MP 以降でオープン |
...
<figure><img src="images/range-table-score-heatmap.png" alt="169ハンドのスコア値を濃淡で示すヒートマップ（AA=38〜72o=7）。しきい値ライン UTG≥24 / MP≥22 / CO≥21 / BTN≥18 / SB≥22 付き" /><figcaption>ハンドスコア ヒートマップ — 高スコア（緑）ほど多くのポジションでオープン可能。グラデーションバーの縦線がポジション別しきい値を示す</figcaption></figure>
...
- ポジション別しきい値：UTG 24 / MP 22 / CO 21 / BTN 18 / SB 22
- スコア ≥ しきい値ならオープン、未満ならフォールド
- 77（スコア24）はUTGしきい値24にちょうど一致する境界の基準点です
