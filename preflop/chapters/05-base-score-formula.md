...
<figure><img src="images/p05-fig4-threshold-ladder.jpg" alt="UTG 23 / MP 22 / CO 20 / BTN 18 / SB 22 のポジション別プレイしきい値ラダー" /><figcaption>UTG 23 / MP 22 / CO 20 / BTN 18 / SB 22 のポジション別プレイしきい値ラダー</figcaption></figure>
...
しきい値は GTO の RFI 頻度（UTG 約 17% 〜 BTN 約 43%）に Score 式の値域を合わせて校正しました。UTG は最もタイト、BTN は最もルース。**SB は表面上 RFI が広く見えますが、これは raise + limp の混合戦略の合計です。本書は「raise か fold」のシンプル戦略を採るため、SB のしきい値は raise レンジ（約 24%）に合わせて 22 と MP 並みに設定しています**。
...
UTGしきい値23を超えるのでUTGオープン可。ただしこれはGTOよりやや積極的で、実戦ではやや控えめに構える判断もあります。
...
<figure><img src="images/range-table-score-heatmap.png" alt="169ハンドのスコア値を濃淡で示すヒートマップ（AA=38〜72o=7）。しきい値ライン UTG≥23 / MP≥22 / CO≥20 / BTN≥18 / SB≥22 付き" /><figcaption>ハンドスコア ヒートマップ — 高スコア（緑）ほど多くのポジションでオープン可能。グラデーションバーの縦線がポジション別しきい値を示す</figcaption></figure>
...
- ポジション別しきい値：UTG 23 / MP 22 / CO 20 / BTN 18 / SB 22
