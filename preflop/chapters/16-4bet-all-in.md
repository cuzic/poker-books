...
本書は**バリュー4ベットのみ**を推奨します。AA、KK、QQ、AKだけで4ベットを打つルールです。
...
有効スタックが20BB以下になると、もはや「レイズ → コール → フロップを見る」という流れは存在しません。プリフロップでオールインするか、フォールドするかの二択になります。

### プッシュハンド早見表（ざっくり）

ポジション別の「プッシュ可能ハンドの目安」（有効スタック15BB前後の例）：。
...
15BB以下になったら、基本的に**「プッシュかフォールド」**の二択だけです。オープンレイズやコールは選択肢から消えます。

詳細なPush/Foldチャートは本書の範囲外です。興味があれば **ICMIZER** などのツールで学習してください（第20章で紹介）。
