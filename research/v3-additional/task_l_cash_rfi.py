#!/usr/bin/env python3
"""task_l_cash_rfi.py — Cash RFI 5 オープナーの hand-level (5 spots)"""
...
SCENARIOS = [
    ("UTG_RFI", ""),         # UTG's turn (first action)
    ("HJ_RFI",  "F"),        # UTG fold → HJ
    ("CO_RFI",  "F-F"),
    ("BTN_RFI", "F-F-F"),
    ("SB_RFI",  "F-F-F-F"),
]
...
def extract(sols):
    ...
    # RFI: only fold / raise (no call for first-in)
    return {h: {"fold": round(fold[i],4), "raise": round(bet[i],4)}
            for h, i in HAND_TO_INDEX.items()}


for label, pf in SCENARIOS:
    out = OUT_DIR / f"task_l_{label}.json"
    if out.exists():
    ...
    time.sleep(0.5)
print(f"\n=== タスク L (Cash RFI) 完了 ({len(SCENARIOS)} シナリオ) ===")
